...
- **`MLCore` (`src/MLCore`)**: Provides supervised and unsupervised machine learning (MLPack backend) for core data types, primarily operating on `TensorData`. Includes dimensionality reduction via `MLDimReductionOperation` (PCA).

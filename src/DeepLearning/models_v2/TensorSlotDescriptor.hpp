#ifndef NEURALYZER_TENSOR_SLOT_DESCRIPTOR_HPP
#define NEURALYZER_TENSOR_SLOT_DESCRIPTOR_HPP
...
/**
 * @brief Fixed batch size where the model is compiled for exactly N samples per call.
 */
struct FixedBatch {
    int size = 1;
};
...
/** 
 * @brief Dynamic batch size where the model accepts a range of batch sizes.
...
/**
 * @brief Recurrent-only mode where the batch size is locked to 1 because the model
 * uses output→input feedback that requires sequential processing.
 */
struct RecurrentOnlyBatch {};

/**
 * @brief Discriminated union describing batch-size constraints for a model.
 *
...
/**
 * @brief Check if a BatchMode forces batch_size = 1.
 */
...
/**
 * @brief Get the maximum batch size allowed by a BatchMode.
 * @return Maximum batch size, or 0 for unlimited.
 */
...
/**
 * @brief Get the minimum batch size required by a BatchMode.
 * @return Minimum batch size, or 1 for RecurrentOnlyBatch.
 */
...
/**
 * @brief Get a human-readable description of a BatchMode.
...
/**
 * @brief Tensor data types supported for slot descriptors.
 * 
...
#endif// NEURALYZER_TENSOR_SLOT_DESCRIPTOR_HPP

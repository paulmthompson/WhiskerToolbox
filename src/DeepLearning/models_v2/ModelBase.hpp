...
/**
 * @brief Abstract base class for all v2 deep-learning model wrappers.
 *
...
public:
    ...
    /**
     * @brief Input slot descriptors (ordered).
     *
     * Each describes one named tensor the model expects.
     * Does not include the batch dimension.
     */
    [[nodiscard]] virtual std::vector<TensorSlotDescriptor> inputSlots() const = 0;

    /**
     * @brief Output slot descriptors (ordered).
     *
     * Each describes one named tensor the model produces.
     * Does not include the batch dimension.
     */
    [[nodiscard]] virtual std::vector<TensorSlotDescriptor> outputSlots() const = 0;

    /**
     * @brief Load weights from a file (e.g. a `.pte` ExecuTorch program or a TorchScript model).
     */
    ...
    /**
     * @brief Preferred batch size.
     *
     * 0 = model decides; 1 = single-frame (e.g. NeuroSAM with output→input
     * feedback); N = process N frames at once. UI uses this as the default
     * for the "Batch Size" spinbox.
     */
    [[nodiscard]] virtual int preferredBatchSize() const { return 0; }

    /**
     * @brief Maximum batch size the model supports.
     *
     * 0 = unlimited.
     */
    ...
    /**
     * @brief Batch mode describing the model's batch-size constraints.
     *

...
std::optional<std::string> PipelineValueStore::getJson(std::string const & key) const {
    ...
    return std::visit([](auto const & val) -> std::string {
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            return std::format("\"{}\"", val);
        } else if constexpr (std::is_same_v<T, ClockTicks>) {
            return std::format("{}", val.getValue());
        }
    },
                      it->second);
}
...
std::optional<ClockTicks> PipelineValueStore::getClockTicks(std::string const & key) const {
    auto it = values_.find(key);
    if (it == values_.end()) {
        return std::nullopt;
    }

    return std::visit([](auto const & val) -> std::optional<ClockTicks> {
        using T = std::decay_t<decltype(val)>;
        if constexpr (std::is_same_v<T, ClockTicks>) {
            return val;
        } else if constexpr (std::is_same_v<T, int64_t>) {
            return ClockTicks{val};
        } else {
            return std::nullopt;
        }
    },

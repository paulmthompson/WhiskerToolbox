...
TEST_CASE("Mask principal axis calculation - Core functionality", "[mask][principal_axis][transform]") {
    ...
    SECTION("Horizontal line mask - major axis should be horizontal") {
        ...
        mask_data->addAtTime(TimeFrameIndex(20), Mask2D(x_coords, y_coords), NotifyObservers::No);
    ...
    SECTION("Vertical line mask - major axis should be vertical") {
        ...
        mask_data->addAtTime(TimeFrameIndex(30), Mask2D(x_coords, y_coords), NotifyObservers::No);
    ...
    SECTION("Diagonal line mask - test axis calculation") {
        ...
        mask_data->addAtTime(TimeFrameIndex(40), Mask2D(x_coords, y_coords), NotifyObservers::No);
    ...
    SECTION("Rectangle mask - test major vs minor axis") {
        ...
        mask_data->addAtTime(TimeFrameIndex(50), Mask2D(x_coords, y_coords), NotifyObservers::No);
    ...
    SECTION("Multiple masks at one timestamp") {
        ...
        mask_data->addAtTime(TimeFrameIndex(60), Mask2D(x1, y1), NotifyObservers::No);

        // Second mask - vertical line
        std::vector<uint32_t> x2 = {5, 5, 5, 5};
        std::vector<uint32_t> y2 = {0, 1, 2, 3};
        mask_data->addAtTime(TimeFrameIndex(60), Mask2D(x2, y2), NotifyObservers::No);
...
TEST_CASE("MaskPrincipalAxisOperation - Operation interface", "[mask][principal_axis][operation]") {
    ...
    SECTION("Execute operation with minor axis") {
        ...
        mask_data->addAtTime(TimeFrameIndex(60), Mask2D(x, y), NotifyObservers::No);
...
TEST_CASE("Data Transform: Mask Principal Axis - JSON pipeline", "[mask][principal_axis][transform][json]") {
    ...
    test_mask->addAtTime(TimeFrameIndex(200), Mask2D(x_coords_vert, y_coords_vert), NotifyObservers::No);
    
    // Add diagonal line mask at time 300
    std::vector<uint32_t> x_coords_diag = {0, 1, 2, 3, 4};
    std::vector<uint32_t> y_coords_diag = {0, 1, 2, 3, 4};
    test_mask->addAtTime(TimeFrameIndex(300), Mask2D(x_coords_diag, y_coords_diag), NotifyObservers::No);
    ...
    rectangle_mask->addAtTime(TimeFrameIndex(400), Mask2D(x_rect, y_rect), NotifyObservers::No);

...
std::shared_ptr<DigitalIntervalSeries> interval_threshold(
    ...
    auto addIntervalIfValid = [&intervals, minDuration](TimeFrameIndex start, TimeFrameIndex end) {
        ...
        if (static_cast<double>(end.getValue() - start.getValue() + 1) >= minDuration) {
            intervals.push_back(TimeFrameInterval{start, end});
        }
    };
    ...
    for (size_t i = 0; i < total_samples; ++i) {
        ...
        if (i > 0 && thresholdParams.missingDataMode == IntervalThresholdParams::MissingDataMode::TREAT_AS_ZERO) {
            ...
            if (actual_step > (typical_time_step * 3 / 2)) {
                ...
                if (in_interval && !zeroMeetsThreshold) {
                    ...
                    auto const gap_start = TimeFrameIndex{static_cast<int64_t>(prev_time)};
                    addIntervalIfValid(interval_start, gap_start);
                    last_interval_end = gap_start.getValue();
                    in_interval = false;
                ...
                if (in_interval && zeroMeetsThreshold) {
                    last_valid_time = TimeFrameIndex{static_cast<int64_t>(curr_time)} - TimeFrameIndex{static_cast<int64_t>(typical_time_step)};
                } else {
        ...
        if (threshold_met && !in_interval) {
            ...
            last_interval_end = interval_end.getValue();
            in_interval = false;
        }

...
TEST_CASE("DM - TV - LineLengthComputer basic integration", "[LineLengthComputer]") {
    ...
    {
        std::vector<float> xs = {0.0f, 10.0f, 10.0f, 0.0f};
        std::vector<float> ys = {0.0f, 0.0f, 10.0f, 10.0f};
        lineData->emplaceAtTime(TimeFrameIndex(0), xs, ys);
    }
    // simple line: (0,0) -> (5,0) (length = 5)
    {
        std::vector<float> xs = {0.0f, 5.0f};
        std::vector<float> ys = {0.0f, 0.0f};
        lineData->emplaceAtTime(TimeFrameIndex(1), xs, ys);
    }
    // simple line: (0,0) -> (3,4) (length = 5, 3-4-5 triangle)
    {
        std::vector<float> xs = {0.0f, 3.0f};
        std::vector<float> ys = {0.0f, 4.0f};
    ...
    // Expect 1 column with lengths
    auto names = table.getColumnNames();
    REQUIRE(names.size() == 1);
    REQUIRE(table.hasColumn("Length"));

    // Validate length values
    auto const & lengths_col = table.getColumnValues<float>("Length");
    REQUIRE(lengths_col.size() == 3);
    REQUIRE(lengths_col[0] == Catch::Approx(30.0f));// perimeter of rectangle
    REQUIRE(lengths_col[1] == Catch::Approx(5.0f)); // horizontal line
    REQUIRE(lengths_col[2] == Catch::Approx(5.0f)); // 3-4-5 triangle
}

TEST_CASE("DM - TV - LineLengthComputer can be created via registry", "[LineLengthComputer][Registry]") {
    DataManager dm;
    ...
    // Line 2: (0,0) -> (6,8) (length = 10)
    {
        std::vector<float> xs = {0.0f, 6.0f};
        std::vector<float> ys = {0.0f, 8.0f};
        lineData->emplaceAtTime(TimeFrameIndex(1), xs, ys);
    }
    ...
    auto const & lengths_col = table.getColumnValues<float>("Length");
    REQUIRE(lengths_col.size() == 2);
    REQUIRE(lengths_col[0] == Catch::Approx(5.0f));
    REQUIRE(lengths_col[1] == Catch::Approx(10.0f));
}

TEST_CASE("DM - TV - LineLengthComputer with per-line row expansion", "[LineLengthComputer][Expansion]") {
    DataManager dm;
    ...
    // t=1: one horizontal line from x=0..10 (length = 10)
    {
        std::vector<float> xs = {0.0f, 10.0f};
        std::vector<float> ys = {0.0f, 0.0f};
        lineData->emplaceAtTime(TimeFrameIndex(1), xs, ys);
    }
    // t=2: two lines; l0 horizontal (x 0..10, length=10), l1 vertical (y 0..10, length=10)
    {
        std::vector<float> xs = {0.0f, 10.0f};
        std::vector<float> ys = {0.0f, 0.0f};
        lineData->emplaceAtTime(TimeFrameIndex(2), xs, ys);
    ...
    // t=4: one diagonal line (0,0) -> (3,4) (length = 5)
    {
        std::vector<float> xs = {0.0f, 3.0f};
        std::vector<float> ys = {0.0f, 4.0f};
    ...
    // Validate per-entity length ordering as inserted:
    // Row 0 -> t=1, the single horizontal line: length = 10
    // Row 1 -> t=2, entity 0 (horizontal): length = 10
    // Row 2 -> t=2, entity 1 (vertical):   length = 10
    // Row 3 -> t=4, the single diagonal line: length = 5
    auto const & lengths_col = table.getColumnValues<float>("Length");
    REQUIRE(lengths_col.size() == 4);

    REQUIRE(lengths_col[0] == Catch::Approx(10.0f));
    REQUIRE(lengths_col[1] == Catch::Approx(10.0f));
    REQUIRE(lengths_col[2] == Catch::Approx(10.0f));
    REQUIRE(lengths_col[3] == Catch::Approx(5.0f));
}

TEST_CASE("DM - TV - LineLengthComputer expansion with coexisting analog column", "[LineLengthComputer][Expansion][AnalogBroadcast]") {
    DataManager dm;
    ...
    REQUIRE(table.getRowCount() == 4);
    auto const & lineLengths = table.getColumnValues<float>("LineLength");
    auto const & analog = table.getColumnValues<double>("Analog");
    REQUIRE(lineLengths.size() == 4);
    REQUIRE(analog.size() == 4);

    // At t=1 (row 1), line exists; others should be zeros for line columns
    REQUIRE(lineLengths[0] == Catch::Approx(0.0f)); // t=0, no line
    REQUIRE(lineLengths[1] == Catch::Approx(10.0f));// t=1, line exists
    REQUIRE(lineLengths[2] == Catch::Approx(0.0f)); // t=2, no line
    REQUIRE(lineLengths[3] == Catch::Approx(0.0f)); // t=3, no line
...
/**
 * @brief Base test fixture for LineLengthComputer with realistic line data
 * 
 ...
 */
class LineLengthTestFixture {
protected:
    LineLengthTestFixture() {
...
private:
    ...
    void createGeometricShapes() {
        ...
        // Triangle at t=20 (perimeter ≈ 32.36)
        {
        ...
        // Circle (octagon approximation) at t=40 (circumference ≈ 30.61)
        {
            std::vector<float> xs, ys;
            for (int i = 0; i <= 8; ++i) {
...
TEST_CASE_METHOD(LineLengthTestFixture, "DM - TV - LineLengthComputer with DataManager fixture", "[LineLengthComputer][DataManager][Fixture]") {
    ...
    SECTION("Test with whisker trace data from fixture") {
        ...
        // Add LineLengthComputer
        auto length_computer = std::make_unique<LineLengthComputer>(
                whisker_source, "WhiskerTraces", whisker_time_frame);
        builder.addColumn<float>("Length", std::move(length_computer));

        // Build the table
        TableView table = builder.build();

        // Verify table structure - 1 column for lengths
        ...
        // Get sample data to verify length values
        auto lengths_col = table.getColumnValues<float>("Length");

        REQUIRE(lengths_col.size() == 9);

        // Verify that all lengths are positive (curved lines should have some length)
        for (size_t i = 0; i < lengths_col.size(); ++i) {
            REQUIRE(lengths_col[i] > 0.0f);
        }

        // Verify that lengths are reasonable (not too small, not too large)
        for (size_t i = 0; i < lengths_col.size(); ++i) {
            REQUIRE(lengths_col[i] > 1.0f);   // Should be at least 1 unit long
            REQUIRE(lengths_col[i] < 1000.0f);// Should not be unreasonably long
        }
    }
    ...
    SECTION("Test with geometric shape data and multiple entities") {
        ...
        // Verify lengths match expected values for geometric shapes
        // Square perimeter = 40
        REQUIRE(lengths_col[0] == Catch::Approx(40.0f));

        // Triangle perimeter ≈ 32.36 (√125 + 10 + √125)
        REQUIRE(lengths_col[1] == Catch::Approx(32.36f).margin(0.1f));

        // Circle circumference ≈ 30.61 (octagon approximation of 2πr = 2π*5)
        REQUIRE(lengths_col[2] == Catch::Approx(30.61f).margin(0.5f));

        // Star shape should have some reasonable length
        REQUIRE(lengths_col[3] > 0.0f);
    }
}

TEST_CASE_METHOD(LineLengthTestFixture, "DM - TV - LineLengthComputer via ComputerRegistry", "[LineLengthComputer][Registry]") {

    SECTION("Verify LineLengthComputer is registered in ComputerRegistry") {
        auto & registry = getTableRegistry().getComputerRegistry();

        // Check that LineLengthComputer is registered
        auto line_length_info = registry.findComputerInfo("Line Length");

        REQUIRE(line_length_info != nullptr);

        // Verify computer info details
        REQUIRE(line_length_info->name == "Line Length");
    ...
    SECTION("Create LineLengthComputer via ComputerRegistry") {
        auto & dm = getDataManager();
        auto dme = std::make_shared<DataManagerExtension>(dm);
        auto & registry = getTableRegistry().getComputerRegistry();
        ...
        // Test computer
        {
            TableViewBuilder builder(dme);
            builder.setRowSelector(std::move(row_selector));
            builder.addColumn("RegistryLength", std::move(computer));
            ...
            auto column_names = table.getColumnNames();
            REQUIRE(table.hasColumn("RegistryLength"));

            auto lengths_col = table.getColumnValues<float>("RegistryLength");
            REQUIRE(lengths_col.size() == 2);
            // Both lines should have positive length
            REQUIRE(lengths_col[0] > 0.0f);
            REQUIRE(lengths_col[1] > 0.0f);
        }
    }
    ...
    SECTION("Compare registry-created vs direct-created computers") {
        ...
        auto whisker_time_frame = dm.getTime(TimeKey("whisker_time"));
        auto direct_computer = std::make_unique<LineLengthComputer>(
                whisker_source, "WhiskerTraces", whisker_time_frame);
        ...
        // Registry computer test
        {
            ...
            auto registry_lengths = registry_table.getColumnValues<float>("Registry");
            REQUIRE(registry_lengths.size() == 2);
            REQUIRE(registry_lengths[0] > 0.0f);
            REQUIRE(registry_lengths[1] > 0.0f);
        }
        ...
        // Direct computer test
        {
            ...
            builder.addColumn<float>("Direct", std::move(direct_computer));
            auto direct_table = builder.build();

            auto direct_lengths = direct_table.getColumnValues<float>("Direct");
            REQUIRE(direct_lengths.size() == 2);
            REQUIRE(direct_lengths[0] > 0.0f);
            REQUIRE(direct_lengths[1] > 0.0f);
        }

        std::cout << "Comparison test - Both computers produce positive length values" << std::endl;
    }
}

TEST_CASE_METHOD(LineLengthTestFixture, "DM - TV - LineLengthComputer via JSON TablePipeline", "[LineLengthComputer][JSON][Pipeline]") {

    SECTION("Test basic line length extraction via JSON pipeline") {
        // JSON configuration for testing LineLengthComputer through TablePipeline
        char const * json_config = R"({
            "metadata": {
                "name": "Line Length Test",
                "description": "Test JSON execution of LineLengthComputer",
                "version": "1.0"
            },
            "tables": [
                {
                    "table_id": "line_length_test",
                    "name": "Line Length Test Table",
                    "description": "Test table using LineLengthComputer",
                    "row_selector": {
                        "type": "timestamp",
                    ...
                    "columns": [
                        {
                            "name": "WhiskerLength",
                            "description": "Extract lengths from whisker traces",
                            "data_source": "WhiskerTraces",
                            "computer": "Line Length"
                        }
                    ]
                }
            ]
        ...
        REQUIRE(config.table_id == "line_length_test");
        REQUIRE(config.name == "Line Length Test Table");
        REQUIRE(config.columns.size() == 1);

        // Verify column configuration
        auto const & column = config.columns[0];
        REQUIRE(column["name"] == "WhiskerLength");
        REQUIRE(column["computer"] == "Line Length");
        ...
        if (pipeline_result.success) {
            ...
            REQUIRE(registry.hasTable("line_length_test"));

            // Get the built table and verify its structure
            auto built_table = registry.getBuiltTable("line_length_test");
            ...
            REQUIRE(column_names.size() == 1);
            REQUIRE(built_table->hasColumn("WhiskerLength"));
            ...
            auto lengths_col = built_table->getColumnValues<float>("WhiskerLength");

            REQUIRE(lengths_col.size() == 9);

            // Verify that all lengths are positive (curved whisker traces should have some length)
            for (size_t i = 0; i < lengths_col.size(); ++i) {
                REQUIRE(lengths_col[i] > 0.0f);
                REQUIRE(lengths_col[i] < 1000.0f);// Reasonable upper bound
            }

            std::cout << "✓ All length values are positive and reasonable" << std::endl;

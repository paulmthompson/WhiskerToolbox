...
std::vector<Point2D<float>> order_line(
    ...
    std::transform(line_pixels.begin(), line_pixels.end(), distances_to_origin.begin(), [&origin](Point2D<float> const & p) {
    ...
    while (!line_pixels.empty()) {
        ...
        std::transform(line_pixels.begin(), line_pixels.end(), distances_to_current.begin(), [&current_point](Point2D<float> const & p) {

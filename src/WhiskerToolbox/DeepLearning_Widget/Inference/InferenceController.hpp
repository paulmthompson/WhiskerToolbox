...
/**
 * @brief Orchestrates inference runs and owns the batch worker thread.
 */
...
public:
    /**
     * @brief Constructor for InferenceController.
     *
    ...
    /**
     * @brief Whether a batch inference is currently running.
     */
    [[nodiscard]] bool isRunning() const;

public slots:
    /**
     * @brief Run single-frame inference at the given frame index.
     *
    ...
    /**
     * @brief Run independent (non-recurrent) batch inference over [start, end].
     *
    ...
    /**
     * @brief Run independent batch inference over multiple intervals.
     *
    ...
    /**
     * @brief Run sequential recurrent inference over a frame range.
     *
    ...
    /**
     * @brief Request cancellation of the current batch inference.
     */
    void cancel();

signals:
    /**
     * @brief Emitted during batch inference; Qt queued from worker thread.
     */
    void batchProgressChanged(int current, int total);

    /**
     * @brief Emitted during recurrent inference.
     */
    void recurrentProgressChanged(int current, int total);

    /**
     * @brief Emitted when batch inference completes (success or failure).
     *

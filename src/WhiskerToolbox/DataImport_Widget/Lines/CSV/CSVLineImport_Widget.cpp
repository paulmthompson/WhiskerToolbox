#include "CSVLineImport_Widget.hpp"
#include "ui_CSVLineImport_Widget.h"

#include <QPushButton>
#include <QFileDialog>
#include <QRadioButton>
#include <QLineEdit>
#include <QComboBox>
#include <QSpinBox>
#include <QCheckBox>
#include <QLabel>
#include <QMessageBox>

CSVLineImport_Widget::CSVLineImport_Widget(QWidget * parent)
    : QWidget(parent),
      ui(new Ui::CSVLineImport_Widget) {
    ui->setupUi(this);

    // Connect signals
    connect(ui->single_file_radio, &QRadioButton::toggled, this, &CSVLineImport_Widget::_onLoadModeChanged);
    connect(ui->multi_file_radio, &QRadioButton::toggled, this, &CSVLineImport_Widget::_onLoadModeChanged);
    connect(ui->browse_button, &QPushButton::clicked, this, &CSVLineImport_Widget::_onBrowseButtonClicked);
    connect(ui->load_button, &QPushButton::clicked, this, &CSVLineImport_Widget::_onLoadButtonClicked);

    // Initialize UI state
    _updateUIForLoadMode();
}

CSVLineImport_Widget::~CSVLineImport_Widget() {
    delete ui;
}

void CSVLineImport_Widget::_onLoadModeChanged() {
    _updateUIForLoadMode();
}

void CSVLineImport_Widget::_onBrowseButtonClicked() {
    QString selectedPath;
    
    if (ui->single_file_radio->isChecked()) {
        // Single file mode - open file dialog
        selectedPath = QFileDialog::getOpenFileName(
            this,
            "Select CSV File",
            "",
            "CSV Files (*.csv);;All Files (*)");
    } else {
        // Multi file mode - open directory dialog
        selectedPath = QFileDialog::getExistingDirectory(
            this,
            "Select Directory Containing CSV Files",
            "",
            QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    }
    
    if (!selectedPath.isEmpty()) {
        ui->file_path_edit->setText(selectedPath);
    }
}

void CSVLineImport_Widget::_onLoadButtonClicked() {
    QString filePath = ui->file_path_edit->text().trimmed();
    
    if (filePath.isEmpty()) {
        QMessageBox::warning(this, "No Path Selected", "Please select a file or directory to load.");
        return;
    }
    
    nlohmann::json config;
    
    // Get delimiter (common to both modes)
    QString delimiterText = ui->delimiter_combo->currentText();
    if (delimiterText == "Comma") {
        config["delimiter"] = ",";
    } else if (delimiterText == "Space") {
        config["delimiter"] = " ";
    } else if (delimiterText == "Tab") {
        config["delimiter"] = "\t";
    } else {
        config["delimiter"] = ","; // Default
    }
    
    config["has_header"] = ui->has_header_checkbox->isChecked();
    
    if (ui->single_file_radio->isChecked()) {
        // Single file mode
        config["filepath"] = filePath.toStdString();
        config["multi_file"] = false;
        
        // Get coordinate delimiter (single-file specific)
        QString coordDelimiterText = ui->coordinate_delimiter_combo->currentText();
        if (coordDelimiterText == "Comma") {
            config["coordinate_delimiter"] = ",";
        } else if (coordDelimiterText == "Space") {
            config["coordinate_delimiter"] = " ";
        } else if (coordDelimiterText == "Tab") {
            config["coordinate_delimiter"] = "\t";
        } else {
            config["coordinate_delimiter"] = ","; // Default
        }
        
        config["header_identifier"] = ui->header_identifier_edit->text().toStdString();
        
        emit loadSingleFileCSVRequested("csv", config);
        
    } else {
        // Multi file mode
        config["parent_dir"] = filePath.toStdString();
        config["multi_file"] = true;
        config["x_column"] = ui->x_column_spinbox->value();
        config["y_column"] = ui->y_column_spinbox->value();
        
        // Validate column indices
        if (config["x_column"] == config["y_column"]) {
            QMessageBox::warning(this, "Invalid Column Configuration", 
                                "X and Y columns cannot be the same. Please select different column indices.");
            return;
        }
        
        emit loadMultiFileCSVRequested("csv", config);
    }
}

void CSVLineImport_Widget::_updateUIForLoadMode() {
    bool isSingleFile = ui->single_file_radio->isChecked();
    
    // Update label text
    if (isSingleFile) {
        ui->label_file_path->setText("CSV File:");
        ui->file_path_edit->setPlaceholderText("Select CSV file...");
    } else {
        ui->label_file_path->setText("Directory:");
        ui->file_path_edit->setPlaceholderText("Select directory containing CSV files...");
    }
    
    // Column controls are only relevant for multi-file mode
    ui->label_x_column->setEnabled(!isSingleFile);
    ui->x_column_spinbox->setEnabled(!isSingleFile);
    ui->label_y_column->setEnabled(!isSingleFile);
    ui->y_column_spinbox->setEnabled(!isSingleFile);
    
    // Coordinate delimiter and header identifier are only relevant for single-file mode
    ui->label_coordinate_delimiter->setEnabled(isSingleFile);
    ui->coordinate_delimiter_combo->setEnabled(isSingleFile);
    ui->label_header_identifier->setEnabled(isSingleFile);
    ui->header_identifier_edit->setEnabled(isSingleFile);
    
    // Header checkbox is relevant for both modes
    ui->has_header_checkbox->setEnabled(true);
    
    // Clear the path when switching modes
    ui->file_path_edit->clear();
}

...
MaskImport_Widget::MaskImport_Widget(std::shared_ptr<DataManager> data_manager, QWidget * parent)
    : QWidget(parent),
      ui(new Ui::MaskImport_Widget),
      _data_manager{std::move(data_manager)} {
    ui->setupUi(this);

    // Connect loader type combo box
    connect(ui->loader_type_combo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &MaskImport_Widget::_onLoaderTypeChanged);

    // Connect signals from HDF5 import widget
    connect(ui->hdf5_mask_import_widget, &HDF5MaskImport_Widget::loadSingleHDF5MaskRequested,
            this, &MaskImport_Widget::_handleSingleHDF5LoadRequested);
    connect(ui->hdf5_mask_import_widget, &HDF5MaskImport_Widget::loadMultiHDF5MaskRequested,
            this, &MaskImport_Widget::_handleMultiHDF5LoadRequested);

    // Connect signals from Image import widget
    connect(ui->image_mask_import_widget, &ImageMaskImport_Widget::loadImageMaskRequested,
            this, &MaskImport_Widget::_handleImageMaskLoadRequested);

    // Set initial state of stacked widget based on combo box
    if (ui->loader_type_combo->currentText() == "HDF5") {
        ui->stacked_loader_options->setCurrentWidget(ui->hdf5_mask_import_widget);
    } else if (ui->loader_type_combo->currentText() == "Image") {
        ui->stacked_loader_options->setCurrentWidget(ui->image_mask_import_widget);
    } else {
        _onLoaderTypeChanged(ui->loader_type_combo->currentIndex());
    }
}

MaskImport_Widget::~MaskImport_Widget() {
    delete ui;
}

void MaskImport_Widget::_onLoaderTypeChanged(int index) {
...
void MaskImport_Widget::_handleSingleHDF5LoadRequested() {
...
void MaskImport_Widget::_handleMultiHDF5LoadRequested(QString pattern) {
...
void MaskImport_Widget::_loadMultiHDF5MaskFiles(QString const & dir_name, QString const & pattern_str) {
...
void MaskImport_Widget::_loadSingleHDF5MaskFile(std::string const & filename, std::string const & mask_suffix) {
...
void MaskImport_Widget::_handleImageMaskLoadRequested(QString format, nlohmann::json config) {
    auto mask_key = ui->data_name_text->text().toStdString();
    if (mask_key.empty()) {
        mask_key = "mask";
    }
    ...
    try {
        ...
        if (directory_path.empty()) {
            QMessageBox::critical(this, "Import Error", "Directory path is required for image mask loading.");
            return;
        }
        ...
        // Extract the MaskData from the variant
        if (!std::holds_alternative<std::shared_ptr<MaskData>>(result.data)) {
            QMessageBox::critical(this, "Import Error", "Unexpected data type returned from image loader");
            return;
        }

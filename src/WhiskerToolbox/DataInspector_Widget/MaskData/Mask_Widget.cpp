...
// DISABLED:     ui->tableView->setModel(_mask_table_model.get());
// DISABLED:     ui->tableView->setSelectionBehavior(QAbstractItemView::SelectRows);
// DISABLED:     ui->tableView->setSelectionMode(QAbstractItemView::ExtendedSelection);
// DISABLED:     ui->tableView->setEditTriggers(QAbstractItemView::NoEditTriggers);
// DISABLED:     ui->tableView->setContextMenuPolicy(Qt::CustomContextMenu);

    connect(ui->load_sam_button, &QPushButton::clicked, this, &Mask_Widget::_loadSamModel);
// DISABLED:     connect(ui->tableView, &QTableView::doubleClicked, this, &Mask_Widget::_handleTableViewDoubleClicked);
// DISABLED:     connect(ui->tableView, &QTableView::customContextMenuRequested, this, &Mask_Widget::_showContextMenu);
...
void Mask_Widget::_handleTableViewDoubleClicked(QModelIndex const & index) {
    ...
    auto tf = _data_manager->getData<MaskData>(_active_key)->getTimeFrame();
    if (!tf) {
        std::cout << "Mask_Widget::_handleTableViewDoubleClicked: TimeFrame not found"
...
// DISABLED:     context_menu.exec(ui->tableView->mapToGlobal(position));
}

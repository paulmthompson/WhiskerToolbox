...
void DigitalEventSeriesDataView::setActiveKey(std::string const & key) {
    ...
    if (event_data) {
        _table_model->setEvents(event_data.get());
        _callback_id = event_data->addObserver([this]() { _onDataChanged(); });
    } else {
        _table_model->setEvents(nullptr);
    }
}
...
void DigitalEventSeriesDataView::setGroupManager(GroupManager * group_manager) {
    if (_group_manager) {
        disconnect(_group_manager, nullptr, this, nullptr);
    }
    ...
    if (_group_manager) {
        connect(_group_manager, &GroupManager::groupCreated,
                this, &DigitalEventSeriesDataView::_onGroupChanged);
...
void DigitalEventSeriesDataView::setGroupFilter(int group_id) {
    if (_table_model) {
        _table_model->setGroupFilter(group_id);
    }
}

void DigitalEventSeriesDataView::clearGroupFilter() {
    if (_table_model) {
...
void DigitalEventSeriesDataView::_connectSignals() {
    ...
    connect(_table_view, &QTableView::customContextMenuRequested,
            this, &DigitalEventSeriesDataView::_showContextMenu);
}
...
void DigitalEventSeriesDataView::_onGroupChanged() {
    updateView();
}

std::vector<EntityId> DigitalEventSeriesDataView::getSelectedEntityIds() const {
...
void DigitalEventSeriesDataView::_showContextMenu(QPoint const & position) {
    ...
    auto move_callback = [this](std::string const & target_key) {
        emit moveEventsRequested(target_key);
    };

    auto copy_callback = [this](std::string const & target_key) {
        emit copyEventsRequested(target_key);
    };

    add_move_copy_submenus<DigitalEventSeries>(&context_menu, dataManager().get(), _active_key, move_callback, copy_callback);
    ...
    if (_group_manager) {
        ...
        connect(remove_from_group_action, &QAction::triggered,
                this, &DigitalEventSeriesDataView::removeEventsFromGroupRequested);
    }

    // Add separator and delete operation
    context_menu.addSeparator();
    QAction * delete_action = context_menu.addAction("Delete Selected Event");
    connect(delete_action, &QAction::triggered,
            this, &DigitalEventSeriesDataView::deleteEventsRequested);

    context_menu.exec(_table_view->mapToGlobal(position));
}

void DigitalEventSeriesDataView::_populateGroupSubmenu(QMenu * menu, bool for_moving) {
    if (!_group_manager) {
        return;
    }
    ...
    for (auto it = groups.begin(); it != groups.end(); ++it) {
        ...
        connect(action, &QAction::triggered, this, [this, group_id]() {
            emit moveEventsToGroupRequested(group_id);
        });
    }
}

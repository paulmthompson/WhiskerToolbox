...
TEST_CASE("DigitalIntervalSeriesInspector saves data from DataManager", "[DigitalIntervalSeriesInspector][save]") {
    ensureQApplication();

    auto * app = QApplication::instance();
    REQUIRE(app != nullptr);

    SECTION("Save button exports DigitalIntervalSeries to CSV") {
        auto temp_dir = makeTempDir();
        auto data_manager = std::make_shared<DataManager>();
        data_manager->setOutputPath(temp_dir.string());

        // Create timeframe and interval series
        constexpr int kNumTimes = 100;
        std::vector<int> t(kNumTimes);
        std::iota(t.begin(), t.end(), 0);
        auto tf = std::make_shared<TimeFrame>(t);
        ...
        auto * filename_edit = inspector.findChild<QLineEdit *>("filename_edit");
        REQUIRE(filename_edit != nullptr);
        filename_edit->setText(QStringLiteral("saved_intervals.csv"));
        ...
        auto * csv_saver = inspector.findChild<CSVIntervalSaver_Widget *>("csv_interval_saver_widget");
        ...
        // Verify file content: load back and check intervals

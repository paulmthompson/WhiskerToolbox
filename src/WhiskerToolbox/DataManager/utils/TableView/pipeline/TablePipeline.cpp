...
bool TablePipeline::addColumnToBuilder(TableViewBuilder & builder,
    ...
    // First try to cast as a MultiComputerWrapper<double>
    auto multi_wrapper = dynamic_cast<MultiComputerWrapper<double> *>(computer.get());
    if (multi_wrapper) {
        // Extract the IMultiColumnComputer and add it using addColumns
        auto multi_computer = multi_wrapper->releaseComputer();
        // The wrapper is now empty and can be safely destroyed by the unique_ptr
        builder.addColumns<double>(column_name, std::move(multi_computer));
        return true;
    }

    // Fall back to regular IColumnComputer<double>

...
void AnalogTimeSeries::overwriteAtTimeIndexes(std::vector<float> & analog_data, std::vector<TimeFrameIndex> & time_indices) {
    ...
    for (size_t i = 0; i < time_indices.size(); ++i) {
        ...
        std::optional<DataArrayIndex> data_index = std::visit([&time_indices, i](auto const & time_storage) -> std::optional<DataArrayIndex> {
            if constexpr (std::is_same_v<std::decay_t<decltype(time_storage)>, DenseTimeRange>) {
                ...
                TimeFrameIndex target = time_indices[i];
                ...
                if (target >= start && target <= end) {
                    ...
                    size_t offset = static_cast<size_t>(target.getValue() - start.getValue());
                    return DataArrayIndex(offset);
                }
                return std::nullopt;
            } else {
                // For sparse storage, search for the TimeFrameIndex
                auto it = std::find(time_storage.time_frame_indices.begin(), time_storage.time_frame_indices.end(), time_indices[i]);

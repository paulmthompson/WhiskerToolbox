    ...
    if (index != -1) {
        ...
        for(const auto& key : intervalKeys) {
            if (QString::fromStdString(key) == _training_interval_key) {
                ...
                if (validIdx != -1) ui->trainingIntervalComboBox->setCurrentIndex(validIdx); else ui->trainingIntervalComboBox->setCurrentIndex(0);
                break;
            }
        }
...
void ML_Widget::_fitModel() {
    ...
    if (balancing_flag && !labels.empty()) {
        ...
        else std::cout << "Class balancing disabled or skipped - using original data distribution." << std::endl;
    }
...
// New _createFeatureMatrix method

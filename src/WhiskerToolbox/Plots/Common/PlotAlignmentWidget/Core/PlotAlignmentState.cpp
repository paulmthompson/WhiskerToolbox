...
bool PlotAlignmentState::getPreventOverlap() const {
    return _data.prevent_overlap;
}

void PlotAlignmentState::setPreventOverlap(bool prevent_overlap) {
    if (_data.prevent_overlap != prevent_overlap) {
        _data.prevent_overlap = prevent_overlap;
        emit preventOverlapChanged(prevent_overlap);
    }
}

...
PSTHState::PSTHState(QObject * parent)
    ...
    // Sync initial vertical axis data from member state and keep view_state Y bounds in sync
...
bool PSTHState::fromJson(std::string const & json) {
    ...
    if (result) {
        ...
        // Restore vertical axis state from serialized data and keep view_state Y bounds in sync

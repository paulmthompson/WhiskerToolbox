...
TEST_CASE("AnalogVertexCache: setVertices", "[AnalogVertexCache]") {
    ...
    vertices.reserve(50);
    for (int i = 0; i < 50; ++i) {
        vertices.push_back({ClockTicks(i), static_cast<float>(i * 2), TimeFrameIndex{i}});
    }
...
TEST_CASE("AnalogVertexCache: appendVertices", "[AnalogVertexCache]") {
    ...
    initial.reserve(50);
    for (int i = 0; i < 50; ++i) {
        initial.push_back({ClockTicks(i), static_cast<float>(i * 2), TimeFrameIndex{i}});
    }
    ...
    for (int i = 50; i < 60; ++i) {
        append.push_back({ClockTicks(i), static_cast<float>(i * 2), TimeFrameIndex{i}});
    }
...
TEST_CASE("AnalogVertexCache: prependVertices", "[AnalogVertexCache]") {
    ...
    for (int i = 40; i < 50; ++i) {
        prepend.push_back({ClockTicks(i), static_cast<float>(i * 2), TimeFrameIndex{i}});
    }
...
TEST_CASE("AnalogVertexCache: getVerticesForRange", "[AnalogVertexCache]") {
    ...
    vertices.reserve(10);
    for (int i = 0; i < 10; ++i) {
        vertices.push_back({ClockTicks(i), static_cast<float>(i * 10), TimeFrameIndex{i}});
    }
    ...
    SECTION("Uncached range returns empty") {
        auto flat = cache.getVerticesForRange(TimeFrameIndex{100}, TimeFrameIndex{200}, ClockTicks(0));
        CHECK(flat.empty());
    }
    ...
    SECTION("X uses mapper physical coordinate, not series TimeFrameIndex") {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            mapped.push_back({ClockTicks(physical_x), static_cast<float>(i + 1), TimeFrameIndex{i}});
        }
...
TEST_CASE("AnalogVertexCache: setVertices with zero-capacity does not crash", "[AnalogVertexCache]") {
    // Regression test: when the series timeframe range collapses to zero
    // (e.g., a single data point at time 0 then a huge gap), the cache
    // capacity could be computed as 0. push_back on a zero-capacity
    // circular buffer is a no-op, so m_vertices stays empty. Accessing
    // front()/back() on the empty buffer caused a segfault.
    AnalogVertexCache cache;
    cache.initialize(0);

    std::vector<CachedAnalogVertex> vertices;
    vertices.push_back({ClockTicks(1454), 13.72f, TimeFrameIndex{0}});

    // Must not crash
    cache.setVertices(vertices, TimeFrameIndex{0}, TimeFrameIndex{0});

    // Buffer could not store the vertex, so the cache should be invalid
    CHECK_FALSE(cache.isValid());
    CHECK(cache.size() == 0);
}

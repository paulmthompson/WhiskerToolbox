...
void Mask_Widget::selectPoint(float const x, float const y) {
    ...
    auto const image_height = media->getHeight();
    auto const image_width = media->getWidth();
    ...
    for (size_t i = 0; i < mask_image.size(); i++) {
        if (mask_image[i] > 0) {
            mask.push_back(Point2D<float>{static_cast<float>(i % image_width),
                                          static_cast<float>(i / image_width)});
        }
    }

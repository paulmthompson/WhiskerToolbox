...
const QVector<QColor> GroupManager::DEFAULT_COLORS = {
...
GroupManager::GroupManager(QObject* parent)
    : QObject(parent), m_next_group_id(1) {
}
...
int GroupManager::createGroup(const QString& name, const QColor& color) {
...
bool GroupManager::removeGroup(int group_id) {
    ...
    const auto& point_ids = it->point_ids;
...
const GroupManager::Group* GroupManager::getGroup(int group_id) const {
    auto it = m_groups.find(group_id);
    return (it != m_groups.end()) ? &it.value() : nullptr;
}

bool GroupManager::setGroupName(int group_id, const QString& name) {
    auto it = m_groups.find(group_id);
    if (it == m_groups.end()) {
        return false;
    }
...
bool GroupManager::setGroupColor(int group_id, const QColor& color) {
    auto it = m_groups.find(group_id);
    if (it == m_groups.end()) {
        return false;
    }
...
bool GroupManager::assignPointsToGroup(int group_id, const std::unordered_set<int64_t>& point_ids) {
    auto it = m_groups.find(group_id);
    if (it == m_groups.end()) {
        return false;
    }
...
bool GroupManager::removePointsFromGroup(int group_id, const std::unordered_set<int64_t>& point_ids) {
    auto it = m_groups.find(group_id);
    if (it == m_groups.end()) {
        return false;
    }
...
void GroupManager::ungroupPoints(const std::unordered_set<int64_t>& point_ids) {
    std::unordered_set<int> affected_groups;
...
QColor GroupManager::getPointColor(int64_t point_id, const QColor& default_color) const {

 ...
 * 
 * On macOS, uses OpenGL 4.1 (QOpenGLFunctions_4_1_Core) without compute shaders/SSBO.
 * On other platforms, uses OpenGL 4.3 (QOpenGLFunctions_4_3_Core) with full feature set.
 */
#ifdef __APPLE__
...
struct LineDataVisualization : protected QOpenGLFunctions_4_3_Core {
...
#ifndef __APPLE__
    // Compute shader resources for line intersection (OpenGL 4.3+ only)
...
#ifndef __APPLE__
    QOpenGLBuffer m_selection_mask_buffer;                        // Buffer containing selection mask for each line (OpenGL 4.3+)
#endif
...
#ifndef __APPLE__
    QOpenGLBuffer m_visibility_mask_buffer;           // Buffer containing visibility mask for each line (OpenGL 4.3+)
#endif
     ...
     * 
     * This method uses GPU compute shaders for efficient intersection testing (OpenGL 4.3+).
     * On macOS (OpenGL 4.1), this returns an empty vector.
     */

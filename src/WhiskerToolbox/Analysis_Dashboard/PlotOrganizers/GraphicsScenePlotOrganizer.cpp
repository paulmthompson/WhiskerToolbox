...
bool GraphicsScenePlotOrganizer::removePlot(QString const & plot_id) {
...
PlotContainer * GraphicsScenePlotOrganizer::getPlot(QString const & plot_id) const {
    auto it = plot_containers_.find(plot_id);
...
void GraphicsScenePlotOrganizer::selectPlot(QString const & plot_id) {
...
void GraphicsScenePlotOrganizer::clearAllPlots() {
    ...
    for (QString const & plot_id: plot_ids) {
        removePlot(plot_id);
    }
}
...
void GraphicsScenePlotOrganizer::setDefaultPlotPosition(QPointF const & position) {
    default_position_ = position;
    next_position_ = position;
}

void GraphicsScenePlotOrganizer::arrangeInGrid() {
    qreal const plot_width = 300; // Estimated plot width
    qreal const plot_height = 200;// Estimated plot height
    qreal const spacing = 20;
...
void GraphicsScenePlotOrganizer::onPlotSelected(QString const & plot_id) {
    qDebug() << "GraphicsScenePlotOrganizer::onPlotSelected called with plot_id:" << plot_id;
    emitPlotSelected(plot_id);
}

void GraphicsScenePlotOrganizer::onFrameJumpRequested(int64_t time_frame_index, std::string const & data_key) {

...
#include "common/binary_loaders.hpp"
...
bool BinaryFormatLoader::supportsFormat(std::string const & format, DM_DataType dataType) const {
...
bool BinaryFormatLoader::supportsBatchLoading(std::string const & format,
    ...
    if (format == "binary" && dataType == DM_DataType::Analog) {
        return true;
    }
    return false;
}

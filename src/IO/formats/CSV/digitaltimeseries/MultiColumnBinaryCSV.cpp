 ...
 */
std::vector<TimeFrameInterval> extractIntervalsFromBinaryData(std::vector<int> const & values) {
    ...
    for (size_t i = 0; i < values.size(); ++i) {
        if (values[i] == 1 && !in_interval) {
            ...
            // End of interval
            in_interval = false;
            intervals.push_back(TimeFrameInterval{TimeFrameIndex{static_cast<int64_t>(start_idx)},
                                                  TimeFrameIndex{static_cast<int64_t>(i - 1)}});
        }
    }
    ...
    if (in_interval) {
        intervals.push_back(TimeFrameInterval{TimeFrameIndex{static_cast<int64_t>(start_idx)},
                                              TimeFrameIndex{static_cast<int64_t>(values.size() - 1)}});
    }

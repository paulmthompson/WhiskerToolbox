...
MultiFeatureKalmanFilter::MultiFeatureKalmanFilter(MultiFeatureKalmanConfig config)
    : config_(std::move(config)), kalman_filter_(nullptr), initialized_(false),
      total_state_size_(0), measurement_size_(0) {}
...
GroupTracker::GroupTracker(MultiFeatureKalmanConfig config) : config_(std::move(config)) {}

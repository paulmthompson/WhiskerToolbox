...
void init_timeframe(py::module_ & m) {
    ...
    // --- ClockTicks ---
    py::class_<ClockTicks>(m, "ClockTicks",
                           "Strongly-typed absolute clock tick value from acquisition hardware")
            .def(py::init<int64_t>(), py::arg("value"))
            .def("getValue", &ClockTicks::getValue,
                 "Get the underlying int64 tick value")
            .def(py::self == py::self)
            .def(py::self != py::self)
            .def(py::self < py::self)
            .def(py::self > py::self)
            .def(py::self <= py::self)
            .def(py::self >= py::self)
            .def("__int__", [](ClockTicks const & ticks) {
                return ticks.getValue();
            })
            .def("__index__", [](ClockTicks const & ticks) {
                return ticks.getValue();
            })
            .def("__repr__", [](ClockTicks const & ticks) {
                return "ClockTicks(" + std::to_string(ticks.getValue()) + ")";
            })
            .def("__str__", [](ClockTicks const & ticks) {
                return std::to_string(ticks.getValue());
            });

    py::implicitly_convertible<int64_t, ClockTicks>();
    ...
    py::class_<TimeFrameIndex>(m, "TimeFrameIndex",
            ...
            .def("__str__", [](TimeFrameIndex const & t) {
                return std::to_string(t.getValue());
            });
    ...
    // --- Interval (TimeFrameIndex start/end pair) ---
    py::class_<TimeFrameInterval>(m, "Interval",
                                  "Time interval with TimeFrameIndex start and end")
            .def(py::init([](int64_t start, int64_t end) {
                     return TimeFrameInterval{TimeFrameIndex(start), TimeFrameIndex(end)};
                 }),
                 py::arg("start"), py::arg("end"))
            .def_readwrite("start", &TimeFrameInterval::start)
            .def_readwrite("end", &TimeFrameInterval::end)
            .def("__repr__", [](TimeFrameInterval const & iv) {
                return "Interval(" + std::to_string(iv.start.getValue()) + ", " + std::to_string(iv.end.getValue()) + ")";
            })
            .def(py::self == py::self);
}

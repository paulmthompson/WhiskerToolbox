...
TEST_CASE("LineageRegistry - Basic Operations", "[entity][lineage][registry]") {
    ...
    SECTION("Set and get file origin metadata") {
        FileOrigin origin{
                .m_path = "/tmp/source.csv",
                .m_format = "csv",
                .m_data_type = "PointData",
                .m_source_config_json = R"({"format":"csv"})"};

        registry.setLineageWithFileOrigin("points", Source{}, origin);

        auto stored_origin = registry.getFileOrigin("points");
        REQUIRE(stored_origin.has_value());
        REQUIRE(stored_origin->m_path == "/tmp/source.csv");
        REQUIRE(stored_origin->m_format == "csv");
        REQUIRE(stored_origin->m_data_type == "PointData");

        registry.setLineage("points", OneToOneByTime{"raw_points"});
        REQUIRE_FALSE(registry.getFileOrigin("points").has_value());
    }
}

...
std::shared_ptr<MaskData> apply_binary_image_algorithm(
    ...
    size_t const total_masks = mask_data->getTotalEntryCount();
    ...
    for (auto const & [time, entity_id, mask]: mask_data->flattened_data()) {
        ...
        Image const processed_image = binary_processor(binary_image);

        // Convert processed image back to mask points
        Mask2D const processed_mask = binary_image_to_mask(processed_image);
...
Image mask_to_binary_image(Mask2D const & mask, ImageSize image_size) {
    ...
    return {std::move(image_data), image_size};
}
...
int map_dest_to_source(int dest_coord, int source_size, int dest_size) {
    assert(source_size > 0 && "map_dest_to_source: source_size must be > 0");
    assert(dest_size > 0 && "map_dest_to_source: dest_size must be > 0");
...
Mask2D resize_mask(Mask2D const & mask, ImageSize const & source_size, ImageSize const & dest_size) {
    ...
    for (int dest_y = 0; dest_y < dest_size.height; ++dest_y) {
        int const source_y =
                map_dest_to_source(dest_y, source_size.height, dest_size.height);

        for (int dest_x = 0; dest_x < dest_size.width; ++dest_x) {
            int const source_x =
                    map_dest_to_source(dest_x, source_size.width, dest_size.width);

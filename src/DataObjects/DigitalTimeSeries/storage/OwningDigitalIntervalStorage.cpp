...
OwningDigitalIntervalStorage::OwningDigitalIntervalStorage(std::vector<TimeFrameInterval> intervals) {
...
OwningDigitalIntervalStorage::OwningDigitalIntervalStorage(std::vector<TimeFrameInterval> intervals, std::vector<EntityId> entity_ids) {
...
bool OwningDigitalIntervalStorage::addInterval(TimeFrameInterval const & interval, EntityId entity_id) {
    // Find insertion position by start time
    auto it = std::ranges::lower_bound(_intervals, interval.start, {},
                                       [](TimeFrameInterval const & i) { return i.start; });
...
bool OwningDigitalIntervalStorage::removeInterval(TimeFrameInterval const & interval) {
...
/**
     ...
     */
void OwningDigitalIntervalStorage::setInterval(size_t idx, TimeFrameInterval interval) {
...
std::optional<size_t> OwningDigitalIntervalStorage::findByIntervalImpl(TimeFrameInterval const & interval) const {
...
bool OwningDigitalIntervalStorage::hasIntervalAtTimeImpl(TimeFrameIndex time) const {
...
std::pair<size_t, size_t> OwningDigitalIntervalStorage::getOverlappingRangeImpl(TimeFrameIndex start, TimeFrameIndex end) const {
    ...
    if (!_assume_disjoint_intervals) {
        ...
        for (size_t i = 0; i < _intervals.size(); ++i) {
            TimeFrameInterval const & interval = _intervals[i];
    ...
    auto it_start = std::ranges::lower_bound(_intervals.begin(), it_end, start, {},
                                             [](TimeFrameInterval const & i) { return i.end; });
...
std::pair<size_t, size_t> OwningDigitalIntervalStorage::getContainedRangeImpl(TimeFrameIndex start, TimeFrameIndex end) const {
    if (_intervals.empty() || start > end) {
        return {0, 0};
    }
...
void OwningDigitalIntervalStorage::_sortIntervals() {
    std::ranges::sort(_intervals, [](TimeFrameInterval const & a, TimeFrameInterval const & b) {

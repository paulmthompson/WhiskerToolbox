...
void assignTestTimeFrame(DigitalEventSeries & series, int64_t num_frames = 100) {
    series.setTimeFrame(makeTestTimeFrame(num_frames));
}
}// namespace
...
TEST_CASE("Digital Event Series - Constructor", "[DataManager]") {
    ...
    assignTestTimeFrame(des2);

    // Verify that the constructor sorts the events
    REQUIRE(des2.size() == 3);
    std::vector<ClockTicks> collected;
    for (auto event: des2.view()) {
        collected.push_back(event.time());
    }
    REQUIRE(collected[0] == ClockTicks(1));
    REQUIRE(collected[1] == ClockTicks(2));
    REQUIRE(collected[2] == ClockTicks(3));
}
...
TEST_CASE("Digital Event Series - Add Event", "[DataManager]") {
    ...
    REQUIRE(collected[1] == ClockTicks(2));
    REQUIRE(collected[2] == ClockTicks(3));
    REQUIRE(collected[3] == ClockTicks(5));
}
...
TEST_CASE("Digital Event Series - Remove Event", "[DataManager]") {
    ...
    REQUIRE(collected[0] == ClockTicks(1));
    REQUIRE(collected[1] == ClockTicks(2));
    REQUIRE(collected[2] == ClockTicks(4));
    REQUIRE(collected[3] == ClockTicks(5));
...
TEST_CASE("DigitalEventSeries - Range-based access (C++20)", "[DataManager]") {
    ...
    SECTION("viewTimesInRange returns correct view") {
        ...
        size_t count = 0;
        std::vector<ClockTicks> collected;
        for (ClockTicks event: range) {
            collected.push_back(event);
            count++;
        }

        REQUIRE(count == 4);
        REQUIRE(collected[0] == ClockTicks(2));
        REQUIRE(collected[1] == ClockTicks(3));
        REQUIRE(collected[2] == ClockTicks(5));
        REQUIRE(collected[3] == ClockTicks(7));
    }

    SECTION("viewInRange returns ClockTickWithId objects") {
        ...
        REQUIRE(collected[0] == ClockTicks(3));
        REQUIRE(collected[1] == ClockTicks(5));
        REQUIRE(collected[2] == ClockTicks(7));
    }
}
...
TEST_CASE("DigitalEventSeries - Constructor deduplication", "[DataManager]") {
    SECTION("All duplicates are removed, correct size reported") {
        ...
        assignTestTimeFrame(des);
        REQUIRE(des.size() == 1);
        auto collected = std::vector<ClockTicks>();
        for (auto e: des.view()) { collected.push_back(e.time()); }
        REQUIRE(collected[0] == ClockTicks(5));
    }
    ...
    SECTION("Unsorted input with duplicates is sorted and deduplicated") {
        ...
        REQUIRE(collected[0] == ClockTicks(1));
        REQUIRE(collected[1] == ClockTicks(5));
        REQUIRE(collected[2] == ClockTicks(10));
    }
...
TEST_CASE("DigitalEventSeries - Range interaction with add/remove", "[DataManager]") {
    ...
    REQUIRE(collected.size() == 3);
    REQUIRE(collected[0] == ClockTicks(3));
    REQUIRE(collected[1] == ClockTicks(4));
    REQUIRE(collected[2] == ClockTicks(5));
    ...
    collected = collectRange(TimeFrameIndex(2), TimeFrameIndex(6));
    REQUIRE(collected.size() == 2);
    REQUIRE(collected[0] == ClockTicks(4));
    REQUIRE(collected[1] == ClockTicks(5));
}
...
TEST_CASE("DigitalEventSeries - view() method", "[DataManager][view]") {
    ...
    SECTION("Basic iteration with EventWithId accessors") {
        ...
        std::vector<ClockTicks> collected_values;
        ...
        REQUIRE(collected_times[0] == ClockTicks(1));
        REQUIRE(collected_times[1] == ClockTicks(3));
        REQUIRE(collected_times[2] == ClockTicks(5));
    ...
    SECTION("Direct member access via event_time and entity_id") {
        ...
        REQUIRE(first_event.event_time == ClockTicks(1));
        REQUIRE(first_event.time() == ClockTicks(1));
    }
...
TEST_CASE("DigitalEventSeries - view() concept-compliant iteration", "[DataManager][view]") {
    ...
    SECTION("EventWithId satisfies concept requirements") {
        ...
        REQUIRE(collected_times.size() == 3);
        REQUIRE(collected_times[0] == ClockTicks(10));
        REQUIRE(collected_times[1] == ClockTicks(20));
        REQUIRE(collected_times[2] == ClockTicks(30));

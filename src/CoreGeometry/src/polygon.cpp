...
bool Polygon::containsPoint(Point2D<float> const & point) const {
    ...
    for (size_t i = 0; i < _vertices.size(); ++i) {
        float const xi = _vertices[i].x;
        float const yi = _vertices[i].y;
        float const xj = _vertices[j].x;
        float const yj = _vertices[j].y;
...
static bool isPointInsideLine(Point2D<float> const & point, Point2D<float> const & line_start, Point2D<float> const & line_end) {
    ...
    float const cross_product = (line_end.x - line_start.x) * (point.y - line_start.y) -
...
static Point2D<float> computeLineIntersection(Point2D<float> const & p1, Point2D<float> const & p2,
    ...
    float const denom = (p1.x - p2.x) * (p3.y - p4.y) - (p1.y - p2.y) * (p3.x - p4.x);
    ...
    float const t = ((p1.x - p3.x) * (p3.y - p4.y) - (p1.y - p3.y) * (p3.x - p4.x)) / denom;
...
bool Polygon::intersects(Polygon const & other) const {
    ...
    for (size_t i = 0; i < _vertices.size(); ++i) {
        size_t const next_i = (i + 1) % _vertices.size();
        Point2D<float> const edge1_start = _vertices[i];
        Point2D<float> const edge1_end = _vertices[next_i];

        for (size_t j = 0; j < other._vertices.size(); ++j) {
            size_t const next_j = (j + 1) % other._vertices.size();
            Point2D<float> const edge2_start = other._vertices[j];
            Point2D<float> const edge2_end = other._vertices[next_j];

            // Check if line segments intersect
            float const d1 = (edge2_end.x - edge2_start.x) * (edge1_start.y - edge2_start.y) -
                             (edge2_end.y - edge2_start.y) * (edge1_start.x - edge2_start.x);
            float const d2 = (edge2_end.x - edge2_start.x) * (edge1_end.y - edge2_start.y) -
                             (edge2_end.y - edge2_start.y) * (edge1_end.x - edge2_start.x);
            float const d3 = (edge1_end.x - edge1_start.x) * (edge2_start.y - edge1_start.y) -
                             (edge1_end.y - edge1_start.y) * (edge2_start.x - edge1_start.x);
            float const d4 = (edge1_end.x - edge1_start.x) * (edge2_end.y - edge1_start.y) -

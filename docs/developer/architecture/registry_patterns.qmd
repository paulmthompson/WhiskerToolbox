...
| Format loader registry | Explicit bootstrap | Live format-loader plugins | Capability metadata can carry a schema | File-format import and export |
...
[`LoaderRegistry`](../../../src/IO/core/LoaderRegistry.hpp) is a process-wide
plugin collection for file-format loaders. It owns `IFormatLoader` implementations
and tries them in registration order for a requested `(format, DM_DataType)` pair.
Unlike static registrars, loader registration is explicit:
[`registerAllLoaders()`](../../../src/IO/core/LoaderRegistration.cpp) adds internal
format loaders and any format loaders enabled by the build configuration.

The registry is not itself a generic JSON-to-typed-parameter adapter. The full JSON
configuration is passed to each loader's `load()` or `save()` method. A loader can,
however, advertise each supported format/data-type capability as `LoaderInfo` or
`SaverInfo`; that metadata contains a `ParameterSchema`. Consumers can use the
advertised schema to build an `AutoParamWidget` for format-specific options.

### Strengths

- The registry owns distinct format plugins behind a focused `IFormatLoader`
  interface.
- Capability metadata supports runtime discovery of available load/save formats,
  including batch-loading support.
- Explicit bootstrap supports optional dependencies such as HDF5 and OpenCV
  without static-initialization or whole-archive requirements.
- The format plugin remains responsible for interpreting its own configuration
  schema and file semantics.

### Costs

- Registration order determines which compatible loader is tried first; the
  registry has no duplicate `(format, data type)` conflict policy.
- The registry passes untyped JSON to loaders, so parsing and validation are not
  automatically unified across loader implementations.
- Every application composition root must initialize the loader catalogue before
  using it.

Use this pattern when implementations are selected by an external file format and
data type, rather than by a typed operation signature. Keep a loader's schema in
its capability metadata so import/export UI can remain schema-driven. See
[File IO](../DataManager/file_io.qmd) and the
[AutoParamWidget IO Migration Roadmap](../IO/AutoParamWidget_Migration_Roadmap.qmd).

### Future direction

The IO migration roadmap is the reference design for making `LoaderRegistry`
capabilities the source of truth for the import and export UI. It replaces
format-specific widget factories with a generic flow: query `LoaderInfo` or
`SaverInfo`, load the selected capability's schema into `AutoParamWidget`, and
send the resulting JSON to `tryLoad()` or `trySave()`. Keep this registry section
and that roadmap aligned when the refactor changes registration or metadata
ownership.
...
3. **File-format plugins selected by format and data type:** use an explicitly
   bootstrapped plugin registry like `LoaderRegistry`; expose capability metadata
   and a parameter schema rather than placing format rules in the UI.
4. **Library-owned extensions that must be discovered without editing a central
...
5. **Typed pipeline composition, compatibility queries, or optimized dynamic
   execution:** use the richer `ElementRegistry` only when those capabilities are
   actual requirements.
6. **Stateful trained objects:** use a factory registry like `MLModelRegistry`;

...
| [`IntervalWithAlignmentAdapter`](../../src/GatherResult/IntervalAdapters.hpp) | Plotting / GatherResult | **Implementation detail inside** interval geometry transforms |
...
[`PlotAlignmentGather.hpp`](../../src/WhiskerToolbox/Plots/Common/PlotAlignmentGather.hpp) is the plotting stack’s entry point for “take an alignment series, pick onset/offset (or event time), expand a window, gather source data.” It is a thin wrapper over [`IntervalAdapters.hpp`](../../src/GatherResult/IntervalAdapters.hpp) and [`GatherResult::gather`](../../src/GatherResult/GatherResult.hpp).
...
flowchart TD
    ...
    EEA["gatherWithEventAlignment\nexpandEvents(DES, pre, post)"]
    IWA["gatherWithIntervalAlignment\nwithAlignment(DIS, anchor [, pre, post])"]
    GR["GatherResult::create(source, adapter)"]
...
| Plot API | Adapter | Gather bounds | Alignment time |
|----------|---------|---------------|----------------|
| `gatherWithEventAlignment` | `EventExpanderAdapter` | `[T−pre, T+post]` per event | event time `T` |
| `gatherWithIntervalAlignment` (no window) | `IntervalWithAlignmentAdapter` | raw interval `[start, end]` | start / end / center index |
| `gatherWithIntervalAlignment` (+ window) | `IntervalWithAlignmentAdapter` (window mode) | `[align_abs−pre, align_abs+post]` | anchor converted to absolute time |
| `createAlignedGatherResult` | dispatches above | `window_size/2` symmetric pre/post | from `PlotAlignmentData` |

**Important:** adapters are **lazy**. They compute windows without materializing a `DigitalIntervalSeries`; the equivalent materialized form is [`createOverlapping()`](../../src/DataObjects/DigitalTimeSeries/Digital_Interval_Series.hpp). Each row `i` still maps to one `AlignedInterval`, even when those windows overlap in absolute time.
...
|--|------------------------------|----------------|------------------------------|
| Geometry | Lazy `IntervalSource` adapters | Materialized `Disjoint` DIS | Materialized **`Overlapping`** DIS or adapters |
| Onset window around DIS | `withAlignment(DIS, Start, pre, post)` | **Not yet** — needs `row_pipeline_json` | Transform outputs bounds DIS + anchor DES, or adapter internally |
| Overlapping windows | Supported via lazy adapter | Was risky on `Disjoint` DIS | **Supported** via [`createOverlapping()`](../../src/DataObjects/DigitalTimeSeries/Digital_Interval_Series.hpp) |
| Cross-timeframe | `GatherResult::create` converts adapter TF → source TF | `prepareIntervalsForGather` converts DIS bounds | Same; preserve `IntervalLayout` on convert |

PR 3 should materialize row-pipeline DIS output as **`Overlapping`** when windows may overlap; use adapters when transforms need lazy evaluation or fused alignment metadata.
...
|-----------|-----------|---------|
| `IntervalWindowAtAlignment` | DIS → gather geometry | Fused `IntervalToEventAtAnchor` + `EventWindowExpand`; may output `Overlapping` DIS or lazy `AlignedInterval` sequence |
...
`IntervalWindowAtAlignment` may use [`IntervalWithAlignmentAdapter`](../../src/GatherResult/IntervalAdapters.hpp) internally for lazy evaluation, or materialize an **`Overlapping`** `DigitalIntervalSeries` of gather bounds. When alignment time differs from window start (onset ±W), reductions that need `alignment_time` may require a companion `DigitalEventSeries` of anchors, an adapter, or a gather-executor extension — see [Alignment metadata gap](#alignment-metadata-gap) below.
...
|--------------|-------------------------|-------|
| Onset ±W around contact | `IntervalToEventAtAnchor(start)` → `EventWindowExpand(pre, post)` | Same as `withAlignment(DIS, Start, pre, post)` |
| Offset ±W | `IntervalToEventAtAnchor(end)` → `EventWindowExpand` | |
| Center ±W | `IntervalToEventAtAnchor(center)` → `EventWindowExpand` | |
| Align to DES, ±W | `EventWindowExpand` only (identity on row if row is already DES) | Same as `expandEvents` |
...
`EventWindowExpand` is the DES-side analogue of `EventExpanderAdapter`: it produces gather geometry (one window per event) without merging. It may output an **`Overlapping`** `DigitalIntervalSeries` when materialized.
...
[`GatherResult`](../../src/GatherResult/GatherResult.hpp) also supports lazy geometry via [`AlignedInterval`](../../src/GatherResult/IntervalAdapters.hpp) and `IntervalSource` adapters — useful for plot parity and zero-copy evaluation.
...
1. Keep [`IntervalWithAlignmentAdapter`](../../src/GatherResult/IntervalAdapters.hpp) internally (lazy path)
2. Output a `DigitalEventSeries` of anchors + `EventWindowExpand` (composed row pipeline)
3. Extend the gather executor with per-row `alignment_times` alongside an `Overlapping` DIS
...
| Lazy `IntervalSource` only | **Still valid** — plot path, zero-copy; not required for overlap safety |
...
- Wire `Overlapping` DIS into existing `buildIntervalPipelineProvider` + `prepareIntervalsForGather` (preserve `IntervalLayout` on TF convert). `IntervalSource` path optional for plot parity / lazy eval.
...
| Cross-timeframe gather (`prepareIntervalsForGather`) | Implemented |
...
| Lazy gather geometry via `IntervalSource` adapters | **Optional** (plots today); not required for overlap safety |
...
- Should `prepareIntervalsForGather` propagate `IntervalLayout::Overlapping` on converted copies?

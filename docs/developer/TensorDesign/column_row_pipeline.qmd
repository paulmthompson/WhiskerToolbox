...
| Iterable binding values | DataManager object or row-pipeline output | `TimePosition` for raster/PSTH row `i` |
...
For example, a spike raster column may gather spikes inside each trial window, then run a normalize-time transform with the contact onset event for that row supplied through the value store as a `TimePosition`.
...
|---|---|---|
| `DigitalEventSeries` | Event row `i` | `TimePosition{event_time, event_series.getTimeFrame()}` |
...
The project already has temporal coordinate carrier types in `TimeFrame/TimeFrame.hpp`:

- `TimeIndexAndFrame`, which pairs a `TimeFrameIndex` with a non-owning `TimeFrame const *`.
- `TimePosition`, which pairs a `TimeFrameIndex` with a `std::shared_ptr<TimeFrame>`.

Phase 5 should reuse one of these instead of introducing a new temporal binding type. Because most DataObjects expose `std::shared_ptr<TimeFrame>` through `getTimeFrame()`, `TimePosition` is the likely first choice for `PipelineValueStore` storage and parameter binding.

`NormalizeTime` should accept a bound temporal value such as `TimePosition` and perform its own conversion to the coordinate system needed for each input row. Legacy `int64_t` absolute-time bindings can remain supported if needed, but new row-iterable bindings should use the typed temporal value.
...
pipeline_value_bindings: ContactOnset DigitalEventSeries[i] -> TimePosition -> row_alignment_time
...
source_key: Spikes
row_pipeline_json: identity or trial window pipeline
pipeline_value_bindings: Contact -> IntervalToEvent(start) -> TimePosition -> row_alignment_time
pipeline_json: NormalizeTime(alignment_time bound to row_alignment_time) + event/bin reduction
```
...
iterable binding:  ContactOnset[i] -> TimePosition -> PipelineValueStore["row_alignment_time"]

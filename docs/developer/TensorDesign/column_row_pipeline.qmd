...
The implemented row sources are data-object based: interval rows from `DigitalIntervalSeries`, timestamp rows from `DigitalEventSeries`, and derived timestamp rows from timestamp-bearing data objects through `extractTimeIndices()`. A missing but important row source is a bare `TimeFrame`.

Planned `TimeFrame` rows should let a tensor have one row per index in a named `TimeFrame` without requiring a synthetic data object:
...
This should be distinct from `row_source.data_key`, because timeframes are DataManager time metadata addressed by `TimeKey`, not normal data object keys. In the TensorDesigner UI this should appear as a separate **TimeFrame Rows** option populated from `DataManager::getTimeFrameKeys()`. This will make the current distinction clearer:

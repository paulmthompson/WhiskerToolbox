...
**Status:** Design target. The underlying GatherResult and interval-window primitives are implemented; TensorDesign `row_pipeline_json` wiring is planned.
...
- `ColumnRecipe::row_pipeline_json`.
- TensorDesign JSON parse/serialize support for `row_pipeline_json`.
- Tensor column provider dispatch based on row-pipeline output type.
- A way to bind row-specific iterable values, such as one normalization event per row, into the column TransformPipeline's `PipelineValueStore`.
...
Current interval-row tensor columns always gather source data over the full row interval. This makes these common per-row features hard or impossible to express in TensorDesign JSON:
...
| Iterable binding values | DataManager object or row-pipeline output | Normalization event for raster/PSTH row `i` |
...
For example, a spike raster column may gather spikes inside each trial window, then run a normalize-time transform with the contact onset event for that row supplied through the value store.
...
4. For row `i`, the iterable binding extracts element `i` and writes one or more values into `PipelineValueStore` under declared keys.
...
iterable binding:  ContactOnset[i] -> PipelineValueStore["row_alignment_time"]
...
The target dispatch for interval-row columns is:
...
5. If the row pipeline produces `DigitalEventSeries` sample times without windows, use timestamp/sample provider behavior or expand to windows with `EventToInterval`, depending on the column recipe.
6. Build any iterable `PipelineValueStore` bindings requested by `pipeline_json`, such as one normalization marker per row.
7. Run `pipeline_json` over gathered rows or sampled timestamps with the row store to produce one value per tensor row.
...
Projection and trial-relative transforms should use values supplied through `PipelineValueStore` and existing TimeFrame primitives such as `convert_time_index()` or physical-time lookups.
...
- How should timestamp-only row-pipeline output choose between sampling, window expansion, or rejection?

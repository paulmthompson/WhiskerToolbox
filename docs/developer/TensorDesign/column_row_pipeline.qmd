...
| `point_xy_at_interval_start` | `PointData` source key, row point selection | two sampled columns with `IntervalToEvent(start)` row pipeline |
| `analog_covariates_at_event` | list of analog source keys, sample event pipeline | one sampled column per analog source |

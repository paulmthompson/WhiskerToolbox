...
**Status:** Partially implemented. GatherResult, interval-window primitives, `ColumnRecipe::row_pipeline_json`, TensorDesign JSON round-trip, identity row-pipeline dispatch, `DigitalIntervalSeries` row-pipeline gather windows, `pipeline_value_bindings` for row-specific `PipelineValueStore` values, `DigitalEventSeries` sample-time row-pipeline dispatch, TimeFrame row sources, `PointCoordinate` keypoint coordinate projection, and Phase 9c column presets with TensorDesigner **Add Preset...** support are implemented. Phase 9g polymorphic preset refactoring is implemented, paving the way for file-based presets and dynamic UI generation. The broader preset file library and user/bundled preset catalog (Phase 10) remain planned.
...
- `TensorDesign` can build lazy-column tensors from interval, timestamp, TimeFrame, and derived timestamp row sources.
...
- `NormalizeDigitalEventSeriesRelative` normalizes gathered `DigitalEventSeries` rows to relative `ClockTicks` storage before reductions such as `EventCountInWindow`.
- `gatherAndExecutePipeline()` can execute JSON-loaded container steps before the terminal range reduction, and can apply a per-row bound pipeline copy when row stores are supplied.
- `DigitalEventSeries` row-pipeline outputs are routed to timestamp/sample providers for interval-row columns without terminal range reductions.
- `ShiftDigitalEventSeries` expresses sample offsets as a normal TransformPipeline step.
- Angle-at-contact-onset point sampling is tested through TensorDesign JSON in the whisker-contact scenario.
- `row_source.time_key` with `row_type: "timeframe"` builds one row per index in a registered TimeFrame.
- `PointCoordinate` converts `PointData` to `RaggedAnalogTimeSeries` for x/y keypoint columns.
...
- Broader preset catalog and bundled/user preset file library.
- UI support for configuring row pipelines.

## Problem

Original interval-row tensor columns always gathered source data over the full row interval. With the implemented `DigitalIntervalSeries` row-pipeline path, onset-window event reductions are now expressible in TensorDesign JSON. With `pipeline_value_bindings`, per-row normalization context is also expressible for row-aligned `DigitalEventSeries` markers. With `DigitalEventSeries` row-pipeline dispatch, point sampling is expressible in row-pipeline JSON. Phase 9c presets cover common authoring patterns; remaining gaps are row-pipeline caching, preset libraries, and richer UI for manually configuring row pipelines.
...
**TensorData materialization stops at scalar float.** `pipeline_json` may produce a gathered `DataObject` (for example a relative `DigitalEventSeries` after `NormalizeDigitalEventSeriesRelative`), but `LazyColumnTensorStorage` column providers must collapse each row to a single `float`. Gather-layer `DataObject` output is valid for intermediate transforms and future GatherResult tooling; only aggregators with `ColumnOutputKind::ScalarFloat` are exposed as tensor column presets.
...
TransformPipeline execution and iterable bindings both use the DataObject `view()` contract. For example, a `DigitalEventSeries` contributes `ClockTicksWithId` elements. Users should not need to choose between entity-carrying and bare-time implementation types; presets and generated raw JSON should select the appropriate registered transform for the pipeline execution mode.

For value projections, `bindValueProjectionV2()` operates on `ElementVariant` types. `ElementVariant` contains `ClockTicks` but not `ClockTicksWithId`; when the source view element is `ClockTicksWithId`, the projection helper passes `element.time()` into the TransformPipeline. Therefore the canonical normalize transform for DigitalEventSeries event times is `NormalizeClockTicksValueV2`. The `EntityId` remains available from the original `ClockTicksWithId` view element outside the TransformPipeline.
...
| Angle at onset | `IntervalToEvent(start)` sample times | Sample-at-event provider behavior |
...
| `ShiftDigitalEventSeries` | Implemented | Apply lag/lead offsets to row-pipeline sample times. |
...
## Row Source Extensions

The implemented row sources include interval rows from `DigitalIntervalSeries`, timestamp rows from `DigitalEventSeries`, TimeFrame rows from registered `TimeKey`s, and derived timestamp rows from timestamp-bearing data objects through `extractTimeIndices()`.

`TimeFrame` rows let a tensor have one row per index in a named `TimeFrame` without requiring a synthetic data object:
...
This is distinct from `row_source.data_key`, because timeframes are DataManager time metadata addressed by `TimeKey`, not normal data object keys. In the TensorDesigner UI this appears as a separate **TimeFrame Rows** option populated from `DataManager::getTimeFrameKeys()`. This makes the row-source distinction clearer:

| UI row mode | Source list | Row identity |
|---|---|---|
| Interval Rows | `DigitalIntervalSeries` data keys | one row per interval |
| Timestamp Rows | `DigitalEventSeries` data keys | one row per event |
| TimeFrame Rows | `TimeKey` entries | one row per TimeFrame index |
| Derived from Source | timestamp-bearing data object keys | one row per timestamp extracted from the data object |

`Derived from Source` should remain available for objects such as `AnalogTimeSeries`, `PointData`, `LineData`, `MaskData`, `DigitalEventSeries`, and `DigitalIntervalSeries`, but the UI should describe it as "derive rows from timestamps in a data object" rather than implying a row TransformPipeline.

## Point And Keypoint Columns

Point/keypoint tables are a motivating case for presets because the low-level representation is simple but verbose. For example, a `PointData` object representing one tracked keypoint often needs two scalar tensor columns, `x` and `y`, at each row time.

The preferred low-level building block is a parameterized TransformsV2 container transform rather than tensor-specific column enums or two separate element transforms:

```text
PointCoordinate: PointData -> RaggedAnalogTimeSeries
parameters: coordinate = "x" | "y"
```

This keeps the implementation in one registry entry with a small coordinate switch, preserves `PointData` as the input object, and produces an analog-family output that existing timestamp sampling and interval reductions can consume.

A one-keypoint tensor column group would expand to two `ColumnRecipe`s:
...
For multiple keypoints, the same idea expands one source key into two columns. For example `Nose` and `Paw` become `Nose_x`, `Nose_y`, `Paw_x`, and `Paw_y`. Users should not need to manually author those four raw pipelines; this should be handled by multi-column presets.

For interval-row analyses, the row pipeline can still determine when the point is sampled. For example, point x/y at contact onset uses `Contact -> IntervalToEvent(start)` as `row_pipeline_json`, then applies `PointCoordinate` to the sampled `PointData` source.

`PointCoordinate` should preserve all point entries as a `RaggedAnalogTimeSeries`: if a `PointData` time contains multiple points, the corresponding ragged analog time contains multiple x or y values. Tensor timestamp sampling currently consumes the first value at a time point, which is appropriate for single-keypoint-per-time data and should be documented or made configurable before using multi-point-per-time data. Interval-row workflows can use normal range reductions such as `MeanValue`, `SumValue`, or `MaxValue` over the coordinate values.
...
`PipelineValueBindingRecipe` has the implemented shape:
...
`row_pipeline_json` describes only the row-aligned gather or sample input. The `DigitalIntervalSeries` gather-window form and `DigitalEventSeries` sample-time form are implemented. `row_pipeline_json` should not also be responsible for carrying a separate normalization marker. If a column needs one marker per row for a transform such as normalize-time, that marker should be loaded into `PipelineValueStore` during per-row `pipeline_json` execution.
...
Presets are the preferred user-facing way to generate common analysis columns without manually typing embedded TransformPipeline JSON. A preset is a recipe generator: it accepts a small, preset-specific parameter set and expands into one or more raw `ColumnRecipe` objects.

Preset invocations are accepted as a JSON authoring format and are expanded by `parseDesignJson()` before provider construction. They are not the canonical saved-analysis format. The reproducible artifact remains the expanded TensorDesign JSON containing ordinary `ColumnRecipe` fields. If a user wants to repeat the exact same analysis later, saving and loading the expanded design is the stable path. Presets are for authoring convenience and error reduction.

Example: spike presence around interval onset:

```json
{
  "preset": "event_presence",
  "row_modifier": "window_around_interval_start",
  "parameters": {
    "output_name": "spike_in_onset_window",
    "source_key": "Spikes",
    "pre": 150,
    "post": 150
  }
...
```json
{
  "preset": "raster_events_relative",
  "row_modifier": "bind_interval_start",
  "parameters": {
    "output_name": "trial_relative_spikes",
    "source_key": "Spikes",
    "store_key": "row_alignment_time",
    "binding_source_key": "Trials"
  }
}
```
...
```cpp
struct ColumnPresetExpansion {
    std::vector<ColumnRecipe> columns;
};
...
struct ColumnRecipePresetDescriptor {
    ...
    std::function<ColumnPresetExpansion(ColumnPresetArgs const &)> expand;
};
```

Presets should expand to a list of columns, not a single `ColumnRecipe`. Single-column presets are the common case, but multi-column presets are required for keypoints and other grouped features.

The preset parameter schema is intentionally friendlier and smaller than raw TensorDesign JSON. Each preset can expose whatever inputs matter for its expansion, such as `source_key`, `output_name`, `pre`, `post`, `name_prefix`, or a list of keypoint keys. The expansion function is responsible for substituting those values into raw `row_pipeline_json`, `pipeline_value_bindings`, and `pipeline_json` fields.

Example multi-column preset intents:

| Preset | Parameters | Expansion |
|---|---|---|
| `point_xy` | one `PointData` source key, name prefix | two columns: x and y |
| `multi_point_xy` | list of `PointData` source keys | two columns per source key |
| `analog_covariates` | list of analog source keys | one sampled column per analog source |

These Column Aggregators can then be seamlessly mixed with a Row Modifier (such as `interval_start` or `bind_interval_start`) in the JSON configuration, allowing flexible combinatorics without monolithic presets.

For complete reusable tables, a separate design/table preset layer is useful:

```cpp
struct DesignPresetExpansion {
    RowSourceSpec row_source;
    std::vector<ColumnRecipe> columns;
};
```

A design preset can set the row source and add a whole feature table, such as a whisker-contact table with curvature, spike count, onset spike presence, angle at onset, and several keypoint x/y columns. This should be treated as a higher-level layer than column presets.

Concrete composite preset example:
...
```json
{
  ...
  "parameters": {
    ...
    "angle_key": "Angle",
    "spike_key": "Spikes",
    "keypoints": ["Nose", "WhiskerBase"]
  }
}
```

The preset expands the shared row source once:
...
It then emits fixed analysis columns:
...
For each key in `keypoints`, it emits two additional sampled columns:

```text
Nose_x_at_onset
Nose_y_at_onset
WhiskerBase_x_at_onset
WhiskerBase_y_at_onset
```

Those keypoint columns all share the same row-pipeline sample geometry:

```json
"row_pipeline_json": "{\"steps\": [{\"step_id\": \"contact_start\", \"transform_name\": \"IntervalToEvent\", \"parameters\": {\"point\": \"start\"}}]}"
```

and differ only in `source_key` and `PointCoordinate` parameterization:

```json
{
  "name": "Nose_x_at_onset",
  "source_key": "Nose",
  "row_pipeline_json": "{\"steps\": [{\"step_id\": \"contact_start\", \"transform_name\": \"IntervalToEvent\", \"parameters\": {\"point\": \"start\"}}]}",
  "pipeline_json": "{\"steps\": [{\"step_id\": \"x\", \"transform_name\": \"PointCoordinate\", \"parameters\": {\"coordinate\": \"x\"}}]}"
}
...
The expansion algorithm is intentionally simple:

```text
columns = fixed_whisker_contact_columns()
for keypoint_key in parameters.keypoints:
    columns.push_back(point_coordinate_column(keypoint_key, "x", row_pipeline_json))
    columns.push_back(point_coordinate_column(keypoint_key, "y", row_pipeline_json))
```

This is why preset expansion must support multiple output columns and why arbitrary-length keypoint lists are better handled by compiled/static preset expansion before attempting a pure JSON-template loop system.
...
Simple JSON-template presets can handle one-column or fixed two-column expansions. More complex cases such as "x/y columns for an arbitrary list of keypoints" may require compiled C++ expansion functions or a template language with loops. Prefer static C++ presets first for complex multi-column expansion, while allowing JSON library files for simple and fixed-shape presets.
...
### Saved Designs After Preset Use

Applying a preset in TensorDesigner should append the expanded raw `ColumnRecipe`s to the current design. Saving the design should write the expanded recipes, not the preset invocation.

For example, applying a `point_xy` preset with `source_key = "Nose"` and `name_prefix = "nose"` should save as ordinary columns:
...
It should not require this to execute:
...
Optional future UI metadata may record which preset created a column group, but that metadata should be informational only. The executable design should remain the expanded raw recipe list.

### Preset ID Policy

Initial preset catalogs should reject duplicate IDs across built-in and user presets. Users who want a lab-specific variation should choose a unique ID, such as `wanglab_point_xy`, rather than shadowing `point_xy`.

This keeps preset listing and expansion deterministic. If user override behavior becomes important later, add explicit namespacing such as `builtin:point_xy`, `user:point_xy`, or `wanglab:point_xy` rather than silent shadowing.
...
The row-pipeline refactor reuses that mechanism rather than adding TensorData-specific context plumbing. The implemented iterable binding layer lives in the TensorColumnBuilders/TransformsV2 integration and is exposed through `ColumnRecipe::pipeline_value_bindings`.

The core contract is:

1. A column recipe identifies an iterable context source through `pipeline_value_bindings`.
...
Binding declarations live on `ColumnRecipe`, beside `pipeline_json`. They are not embedded in `PipelineDescriptor`, because the descriptor remains a reusable transform pipeline that does not know about DataManager keys, tensor rows, or external iterable sources. All pipeline-valued fields, including `row_pipeline_json`, `source_pipeline_json`, and `pipeline_json`, use the same `PipelineDescriptor` schema.
...
Phase 5 binds temporal iterable values as `ClockTicks`. `NormalizeTimeParamsV2::alignment_time` receives `ClockTicks`, not raw `TimeFrameIndex` values.
...
For `NormalizeTime`, the user-facing intent is simply "normalize event time by a row-specific clock tick". The raw transform name for DigitalEventSeries value projections is `NormalizeClockTicksValueV2` because the projection helper extracts `ClockTicks` from `ClockTicksWithId` view elements before transform execution. When the gathered row should remain a `DigitalEventSeries`, use the container transform `NormalizeDigitalEventSeriesRelative`, which writes relative `ClockTicks` storage and preserves event `EntityId`s. Presets should hide these raw transform names when possible.
...
Failure occurs at provider construction when possible. If a lazy source can only be evaluated at provider invocation time, fail with a message that identifies the column, binding source, and store key.

### Motivating Test Cases

Phase 5 has tests using the raster/PSTH normalization pattern.
...
The tests assert that normalized event times are relative to the bound marker, not to the gather window start. They also assert row-count mismatch rejection by using a binding source with a missing event.
...
The ownership is clear: TransformsV2 binds row-specific values into `PipelineValueStore`; TensorDesign selects the row source, gather/sample input, and column recipe; TensorData stores the resulting column values.
...
4. `pipeline_json` runs per row and includes a normalize-time transform, such as `NormalizeDigitalEventSeriesRelative` for event-series rows.
...
5. Build any iterable `PipelineValueStore` bindings requested by `pipeline_value_bindings`, such as one normalization marker per row. Initial support binds one `ClockTicks` value per row from a direct or derived `DigitalEventSeries`.
6. Run `pipeline_json` over gathered rows with the row store to produce one value per tensor row.
7. If the row pipeline produces `DigitalEventSeries` sample times and the column pipeline has no terminal range reduction, route those sample times to timestamp/sample provider behavior.

Planned dispatch extensions are:

1. Optional future expansion may allow recipes to choose whether `DigitalEventSeries` row-pipeline output should sample directly or be expanded to windows with `EventToInterval`.
...
Implemented behavior rejects `DigitalEventSeries` row-pipeline output only when the interval-row column also declares a terminal range reduction, because sample-time output is point geometry rather than gather-window geometry.
...
- Should raw JSON expose low-level transform names such as `NormalizeClockTicksValueV2`, or should TensorDesign provide friendlier aliases/presets for common column workflows?
...
- Direct-key bindings and key-plus-`source_pipeline_json` bindings are implemented for `DigitalEventSeries` sources.
...
|---|---|---|
| `tests/TransformsV2/test_tensor_column_builders.test.cpp` | TransformsV2 column-builder layer | `resolveIntervalGatherWindows()`, identity row pipelines, `DigitalIntervalSeries` row-pipeline output, overlapping windows, row-count rejection, interval-property bypass, provider dispatch, JSON-loaded container steps before reduction, direct/derived `pipeline_value_bindings`, and binding row-count rejection. |
| `tests/TensorDesign/test_tensor_design_builder.cpp` | TensorDesign JSON/build layer | Parse/serialize `row_pipeline_json`, parse/serialize `pipeline_value_bindings`, identity behavior, Phase 4 JSON builds, Phase 5 bound-normalization JSON builds, mixed interval-property/gather columns, cross-timeframe gather windows, and offset compatibility characterization. |
| `tests/TensorDesign/test_tensor_whisker_contact_scenario.cpp` | TensorDesign scenario integration | Builds the whisker-contact JSON design with mean curvature, full-contact spike count, contact-onset spike presence, and angle-at-onset point sampling through `row_pipeline_json`. |

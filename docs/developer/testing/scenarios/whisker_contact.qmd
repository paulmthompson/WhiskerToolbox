...
- **Columns**: mean `Curvature`, spike count in interval, spike presence in a one-frame contact-onset window
...
The same tensor can be built from a [TensorDesigner-compatible JSON envelope](../../TensorDesign/TensorDesignBuilder.qmd) using the Qt-free `TensorDesign` library:
...
out/build/Clang/Release/bin/test_tensor_design "[TensorDesign]"
```

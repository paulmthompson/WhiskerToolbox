...
| `tests/TensorDesign/test_tensor_whisker_contact_scenario.cpp` | TensorDesign JSON integration, including contact-aligned spike windows |
...
The TensorDesign scenario test builds the same feature set plus contact-aligned spike presence from a [TensorDesigner-compatible JSON envelope](../../TensorDesign/TensorDesignBuilder.qmd) using the Qt-free `TensorDesign` library:
...
For future point-sampling and per-row normalization examples (angle at onset, trial-relative rasters, tuning-curve tables), see the [column row pipeline design](../../TensorDesign/column_row_pipeline.qmd) and [roadmap](../../TensorDesign/column_row_pipeline_roadmap.qmd).

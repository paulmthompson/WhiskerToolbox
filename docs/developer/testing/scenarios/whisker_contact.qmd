...
For **onset-aligned windows** and **per-column gather geometry** (spike presence near onset, angle at onset, tuning-curve tables), see the planned [column row pipeline design](../../TensorDesign/column_row_pipeline.qmd). That work is not implemented yet; current JSON always gathers over the full contact interval.

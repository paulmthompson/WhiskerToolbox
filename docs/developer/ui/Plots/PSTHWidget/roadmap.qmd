...
**Status**: Infrastructure exists in `PlottingSVG`. Implementation needed for PSTHWidget.

**Goal**: Export the current PSTH plot to SVG for publication-quality figures.

### 5.1 Existing Infrastructure

The project has SVG export infrastructure in `PlottingSVG` (e.g. `PlottingSVG/SVGExport.hpp` and `SVGSceneRenderer`):

- `PlottingSVG::buildSVGDocument(RenderableScene, SVGExportParams)` — full flat SVG document
- `PlottingSVG::SVGSceneRenderer` — layered SVG via `SVGDocument` and per-batch renderers
- `PlottingSVG::renderSceneToSVG()` — all batch types in draw order (uses `SVG*Renderer` internally)
...
namespace Plots::Export {
    QString exportPlotToSVG(
        ...
        PlottingSVG::SVGExportParams const & params);
}
```

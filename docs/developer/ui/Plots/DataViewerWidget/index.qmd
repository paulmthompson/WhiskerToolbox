...
When many channels are loaded but only a few are visible in the Y viewport (e.g., viewing 4 of 32 channels in `FixedHeight` mode), the rendering pipeline uses **additive culling** to avoid building batches for off-screen series while ensuring that panning never permanently loses rendered data.

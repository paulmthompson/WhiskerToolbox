...
This is used for group-based coloring (see [Group Management](#group-management)) and will support external feature coloring and multi-series rendering in the future.
...
- **User changes axis range** (spinboxes or wheel) → `rangeChanged` updates `x_zoom`/`x_pan` via `LinePlotWidget::wireTimeAxis()` (visible-span editing, not `window_size`)
- **User zooms/pans** → axis range updates silently via `setRangeSilent()` (no feedback loop)
- **`rangeUpdated`** in `LinePlotState` only syncs `_data.time_axis` for serialization; it does not emit `stateChanged()` or mutate `alignment.window_size`
...
- `getSelectedEntities()` — maps selected trial indices to `EntityId`s
...
The group management pattern is shared with [ScatterPlotWidget](../ScatterPlotWidget/index.qmd):

...
Hovering over a heatmap row identifies the `DigitalEventSeries` corresponding to the Y-axis position under the cursor. A tooltip is displayed containing a miniature visualization for that unit:

- A **raster plot** showing individual trial events across aligned trials
- A **PSTH histogram** showing the unit's firing rate profile over relative time

The tooltip is rendered on-demand using the cached `GatherResult` data from the last scene build, avoiding a full scene rebuild on each hover event. See [roadmap Phase 8](roadmap.qmd) for the full implementation plan, including rendering approach and layout.

...
| `colorRangeChanged` | Color range mode or bounds changed || `stateChanged` (inherited) | Any change requiring scene rebuild |
...
```
HeatmapState (alignment + estimation_params + scaling)
        ↓
...
applyScaling(rate_estimate, scaling()) per unit
        ↓
HeatmapRowData (times[], values[], metadata.sample_spacing)
        ↓
...
6. Calls `HeatmapMapper::buildScene()` with the colormap and `HeatmapColorRange` from state
7. Uploads via `_scene_renderer.uploadScene()` and emits `unitCountChanged`
...
4. **Time Axis Range Controls** — `RelativeTimeAxisRangeControls` in a collapsible `Section`.
5. **Y-Axis Range Controls** — `VerticalAxisRangeControls` in a collapsible `Section`.

...
| **C — Existing System** | Handled by an existing non-command system (TransformsV2, LoaderRegistry) and a future command wrapper is planned | No action now; tracked for Phase 3+ |
...
|---|-----------|---------------|
| 1 | **Zero direct `DataManager` mutations** | All `setData()`, `deleteData()`, `getData<T>()->addEvent()`, etc. go through `createCommand()` + `execute()`. Category C verbs (TransformsV2, LoaderRegistry) are intentionally exempt until Phase 3+. |
...
Criteria 2 and 3 require Steps 1.13–1.14 from the [roadmap](roadmap.qmd) to be implemented.
Until then, criterion 1 (verified by code review) is sufficient to mark a widget as refactored.
...
3. **Commands live in `src/DataManager/Commands/`** — All command classes and their params
   structs are defined there. The widget only needs to include `CommandFactory.hpp` and the
...
├── ICommand.hpp              ← interface (already exists)
├── CommandFactory.hpp/.cpp   ← factory + getAvailableCommands() (already exists)
├── CommandContext.hpp         ← runtime context (already exists)
├── MoveByTimeRange.hpp/.cpp  ← concrete command (already exists)
├── CopyByTimeRange.hpp/.cpp  ← concrete command (already exists)
├── AddInterval.hpp/.cpp      ← concrete command (already exists)
├── ForEachKey.hpp/.cpp        ← meta-command (already exists)
├── DeleteByEntityIds.hpp/.cpp ← NEW (identified by audit)
├── AddEvent.hpp/.cpp          ← NEW (identified by audit)
├── RemoveEvent.hpp/.cpp       ← NEW (identified by audit)
...
| DataInspector_Widget | **Audited** | ~30 | ~10 | ~14 | ~6 | ~0 | Not yet |
...
- `Export_Widgets` / `MediaExport` — future Phase 3 (SaveData)
...
| Save to CSV/binary | `_handleSave*Requested()` | **C** | `SaveData` (Phase 3) | Future SaverRegistry command |
...
Commands discovered during the audit that need to be implemented. Ordered by priority
(most commonly used across widgets first).
...
□ 9. Implement the command(s) in src/DataManager/Commands/

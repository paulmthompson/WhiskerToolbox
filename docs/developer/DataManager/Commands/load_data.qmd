...
6. Record source lineage with `FileOrigin` metadata on the session `LineageRegistry`.
7. Return `CommandResult::ok({data_key})` on success with the data key as an affected key.

The recorded `FileOrigin` stores the resolved path, format, data type, and serialized command parameters. Data inspector save handlers use this metadata to default a matching save format back to the original load file.

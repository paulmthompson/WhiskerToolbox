...
The `ObserverData` class manages a collection of callback functions that are invoked when data changes occur. Each instance can carry an optional domain name (used in `spdlog` debug output), and each registered callback can carry an optional observer name for per-callback timing logs.

```cpp
#include "Observer/Observer_Data.hpp"

ObserverData observable("MyDomain");
...
// Trigger all registered callbacks (snapshot iteration; order unspecified)
...
Qt widgets that register observer callbacks must ensure those callbacks run on the main thread when data can be modified from background workers. See the [Observer Thread-Safety Roadmap](observer_thread_safety_roadmap.qmd) for audit status, fix patterns, and the task checklist.
:::

::: {.callout-note}
## Re-entrant Modification During Notification

`notifyObservers()` copies the observer map to a snapshot before invoking callbacks. Adding or removing observers from within a callback is therefore safe for the current round — new registrations take effect on the next `notifyObservers()` call. This re-entrancy support does **not** provide thread safety across threads.
:::
...
| Method | Description |
|--------|-------------|
| `explicit ObserverData(std::string name = "UnnamedObservable")` | Construct with optional domain name for debug logging |
| `[[nodiscard]] CallbackID addObserver(ObserverCallback, std::string name = "Anonymous")` | Register a callback, returns unique ID |
| `void notifyObservers()` | Snapshot registered callbacks and invoke each (order unspecified; logs timing at debug level) |
| `void removeObserver(CallbackID)` | Unregister a callback by ID (no-op if ID is invalid) |

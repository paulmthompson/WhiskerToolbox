...
For `EntityKind::IntervalEntity`, the registry tuple is `(data_key, start, end)`:

- `time` / `time_value` — interval start (`TimeFrameIndex`)
- `local_index` — interval end (not a storage index)

`DigitalIntervalSeries` keys new intervals this way via `ensureId`. When intervals merge (for example, extending an interval end in the DataInspector), the `EntityId` from the earliest-start merged interval is preserved and the registry entry is rebound to the new bounds via `rebindKey`. Reverse lookup uses the `EntityId` stored on the interval (`findByEntityId`), not the registry index.
...
-   Remembering filesystem provenance for loaded source data

### Container Classification

Not all container types carry EntityIds. The table below summarises which containers can have their own identity and which can record parent lineage:
...
**Entity-bearing derived containers** hold both their own EntityIds *and* parent lineage, which serve distinct purposes:

- **Own EntityId**: answers "what is this element's identity?" — used for grouping, selection, and element-specific operations.
- **Parent lineage**: answers "where did this element come from?" — used for provenance, data-flow visualisation, and tracing.

```
// Example: LineData created by skeletonizing MaskData
// MaskData "masks" — Time T0: Mask (EntityId=100)
// LineData "skeleton_lines" — Time T0: Line (EntityId=200)
//
// skeleton_lines.getEntityIdsAtTime(T0) → [200]   (line's own identity)
// resolver.resolveToSource("skeleton_lines", T0)   → [100]   (originating mask)
```
...
`LineageEntry` can also carry optional `FileOrigin` metadata for source data loaded from disk. This metadata is not used by `LineageResolver`; it records where a source key entered the session so save workflows can default back to the original file when the output format matches.

``` cpp
FileOrigin origin{
    .m_path = "/data/session/points.csv",
    .m_format = "csv",
    .m_data_type = "PointData"
};

registry.setLineageWithFileOrigin("points", Source{}, origin);
auto loaded_from = registry.getFileOrigin("points");
```
...
Three concrete components in `DataManager/Lineage/` bridge the abstract Entity/Lineage system to the DataManager storage layer. None of them are part of the Entity library itself; they live in the DataManager library and depend on it.

### DataManagerEntityDataSource

`DataManagerEntityDataSource` (in `DataManager/Lineage/DataManagerEntityDataSource.hpp`) implements `IEntityDataSource` by dispatching on the container's runtime type. All DataManager-specific type knowledge is encapsulated here, keeping `LineageResolver` free of DataManager dependencies.

Supported container types: `MaskData`, `LineData`, `PointData`, `DigitalEventSeries`, `DigitalIntervalSeries`, `AnalogTimeSeries` (RaggedAnalog backend), and `TensorData`.
...
DataManager dm;
WhiskerToolbox::Lineage::DataManagerEntityDataSource data_source(&dm);

// Use directly with LineageResolver when you don't need the EntityResolver wrapper
auto* registry = dm.getLineageRegistry();
WhiskerToolbox::Entity::Lineage::LineageResolver resolver(&data_source, registry);
auto ids = resolver.resolveToSource("mask_areas", TimeFrameIndex(10));
```

`DataManagerEntityDataSource` is non-copyable and non-movable. The `DataManager` pointer it holds is non-owning; callers must ensure the `DataManager` outlives the data source.

### EntityResolver

`EntityResolver` (in `DataManager/Lineage/EntityResolver.hpp`) is a high-level convenience wrapper that composes `DataManagerEntityDataSource` and `LineageResolver` internally. It exposes the same resolution API as `LineageResolver` without requiring callers to manage intermediate objects.
...
// Single-step resolution: derived → immediate source EntityIds
auto ids = resolver.resolveToSource("mask_areas", TimeFrameIndex(5));

// Full-chain resolution: derived → root source(s)
// Handles multi-level chains: MaskData → areas → peaks
auto root_ids = resolver.resolveToRoot("peaks", TimeFrameIndex(5));

// Entity-bearing derived containers: resolve by EntityId
auto parent_ids = resolver.resolveByEntityId("skeleton_lines", EntityId(200));
auto root_entity_ids = resolver.resolveByEntityIdToRoot("skeleton_lines", EntityId(200));

// Bulk queries
auto all_sources = resolver.getAllSourceEntities("mask_areas");
std::vector<std::string> chain = resolver.getLineageChain("peaks");
// chain: ["peaks", "mask_areas", "masks"]
...
`EntityResolver` is the preferred entry point for lineage resolution in code that already holds a `DataManager*`.

### LineageRecorder

`LineageRecorder` (in `DataManager/Lineage/LineageRecorder.hpp`) is a static-method helper that bridges the transform-centric `TransformLineageType` enum (declared in `TransformsV2`) to the storage-centric `Lineage::Descriptor`. It is used after executing a v2 transform pipeline to record what lineage relationship was created.
...
// After executing a transform and storing the result:
...
// For multi-input transforms (e.g., LineMinPointDistance):
LineageRecorder::recordMultiInput(
    *dm->getLineageRegistry(),
    "line_distances",
    {"lines", "points"},
    TransformLineageType::OneToOneByTime
...
// For original/loaded data:
LineageRecorder::recordSource(*dm->getLineageRegistry(), "masks");

// For source data loaded from disk:
LineageRecorder::recordFileSource(*dm->getLineageRegistry(), "masks", origin);
```

Every v2 transform registered in `TransformsV2/core/RegisteredTransforms.cpp` carries a `lineage_type` field in its `TransformMetadata`. Callers can query this field and pass it directly to `LineageRecorder::record()` to avoid hard-coding the lineage type at call sites.
...
| `DataManagerEntityDataSource`| ❌ Not thread-safe | Shares DataManager's thread-safety constraints |
| `EntityResolver`             | ❌ Not thread-safe | Inherits DataManager constraints       |
...
-   `Lineage/LineageResolver.test.cpp` - Resolution with mock data source (40+ test cases)

DataManager-layer unit tests are co-located in `src/DataManager/Lineage/`:

-   `EntityResolver.test.cpp` - Resolution strategies with real DataManager
-   `DataManagerEntityDataSource.test.cpp` - Type dispatch across all supported container types (200+ tests)

Integration tests live in `tests/DataManager/Lineage/`:

-   `test_transform_lineage_integration.test.cpp` - Full pipeline tests (700+ lines): MaskData → MaskArea → SumReduction → EntityResolver
-   `test_lineage_recorder.test.cpp` - LineageRecorder mapping from TransformLineageType to descriptors

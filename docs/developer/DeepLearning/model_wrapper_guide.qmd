  ...
  "outputs": [
    {
      ...
      "recommended_pipeline": [{ "step_id": "TensorToLine2D" }]
    }
  ]
}
```
...
| `recommended_pipeline` | array | No | Ordered output steps, e.g. `[{ "step_id": "TensorToMask2D" }]` |
...
- **`recommended_encoder`** / **`recommended_pipeline`** — the UI pre-selects these but the

...
private:  
    // 1. View State: Manages zoom/pan and time window (ViewStateData in DataViewerState)
    CorePlotting::ViewStateData _view_state;
...
* \[ \] **View State**: Are you using `ViewStateData` (`x_min`/`x_max` as master `TimeFrameIndex`) for X-axis scrolling?  

...
Current interval-row builds gather each column over the full row interval. The planned row-pipeline refactor will let each column run a TransformPipeline over the row source to produce prepared gather windows and optional alignment anchors, then use `GatherResult` as the row-aligned DataObject container for the resulting gathered rows. This keeps TimeFrame conversion and entity identity in DataObjects and TransformPipelines rather than introducing a tensor-specific interval system.
...
- [Column row pipeline design](TensorDesign/column_row_pipeline.qmd) — planned per-column gather geometry via row TransformPipelines

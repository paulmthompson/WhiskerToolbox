...
3. **Runtime gap detection** — The `MutationGuard` automatically logs `DataManager`
   mutations that bypass the command system at runtime. This replaces manual
   header-reading for gap discovery.
4. **Golden trace verification** — A widget is "done" when a recorded command sequence
   replays successfully and produces the expected `DataManagerSnapshot`.

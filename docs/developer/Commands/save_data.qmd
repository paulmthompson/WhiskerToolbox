...
struct SaveDataParams {
    ...
    std::optional<bool> backup_existing;           // Move existing target to backup first
    std::optional<std::string> backup_suffix;      // Backup suffix
};
```
...
| `backup_existing` | Optional. If true, move an existing regular-file target to `path + backup_suffix` before saving |
| `backup_suffix` | Optional. Suffix for the backup path, defaulting to `.bak` |
...
5. If `backup_existing` is true and `path` names an existing regular file, rename it to `path + backup_suffix`.
6. Dispatch through `LoaderRegistry::getInstance().trySave()`.
7. Return `CommandResult::ok()` on success, `CommandResult::error()` on failure.
...
| Backup rename fails | `"Failed to back up existing file ..."` |

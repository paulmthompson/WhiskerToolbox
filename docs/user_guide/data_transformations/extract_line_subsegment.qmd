...
![](images/clipboard-1005583877.png)
...
with plt.xkcd():
    ...
    noisy_line = np.vstack((x, y_noise))
    ...
    # --- Direct Method ---
    total_length = np.sum(np.sqrt(np.sum(np.diff(noisy_line, axis=1)**2, axis=0)))
    start_dist = start_pos * total_length
    end_dist = end_pos * total_length

    cumulative_lengths = np.insert(np.cumsum(np.sqrt(np.sum(np.diff(noisy_line, axis=1)**2, axis=0))), 0, 0)
    ...
    direct_subsegment_points = [get_point_at_distance(noisy_line, start_dist)]

    # Add the original points in between
    for idx in direct_indices:
        # Avoid duplicating the start point if it's an exact match
        if not np.allclose(direct_subsegment_points[-1], noisy_line[:, idx]):
            direct_subsegment_points.append(noisy_line[:, idx])

    # Add the interpolated end point
    end_point = get_point_at_distance(noisy_line, end_dist)
    ...
    ax1.plot(noisy_line[0,:], noisy_line[1,:], 'k--', label='Original Line', alpha=0.5)
    ...
    ax2.plot(noisy_line[0,:], noisy_line[1,:], 'k--', label='Original Line', alpha=0.5)
    ...
    ax2.plot(parametric_subsegment[0,-1], parametric_subsegment[1,-1], 'ro', markersize=10, label='End Point')

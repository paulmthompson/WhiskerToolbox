...
![](images/clipboard-2968981553.png)
...
with plt.xkcd():
    ...
    signal = (np.sin(2 * np.pi * 10 * t) * np.exp(-((t-0.3)**2)/0.002) * 0.8 + # Short burst
              np.sin(2 * np.pi * 10 * t) * np.exp(-((t-0.8)**2)/0.02) * 1.5 + # Long burst
              np.sin(2 * np.pi * 10 * t) * np.exp(-((t-1.1)**2)/0.015) * 1.2) # Second long burst, close to first
    #signal += np.random.randn(len(t)) * 0.1 # Add some noise
    signal /= np.max(np.abs(signal)) # Normalize

    threshold = 0.3
    min_duration_sec = 0.030 # 30 ms
    lockout_sec = 0.300 # 300 ms
    ...
    for i, (start, end) in enumerate(raw_intervals):
        ...
        print(f"Interval {i+1}: Start = {t[start]:.3f}s, End = {t[end]:.3f}s, Duration = {(end-start)/fs*1000:.1f} ms")

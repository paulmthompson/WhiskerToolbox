...
#### Default `time` clock (fallback)

If your JSON does not define `data_type: "time"` with `name: "time"`, Neuralyzer attempts to populate the default `time` TimeFrame **after the full config is loaded**:

- When video (or other media) was loaded and `time` is still empty, an identity frame index vector `[0, 1, …, N-1]` is created from the best available media (any media key—not only `"media"`).
- If you load **only** points, lines, or other non-media data, `time` stays empty unless you add an explicit time entry (for example `format: "max_value"` from an analog or digital event series).

Define `time` explicitly in JSON when you need a custom clock, a derived timebase, or a points-only session without video.

### Forward-declared clocks

When multiple data objects share a custom clock (for example a DAQ master clock), declare clock names at the top of the JSON object before the `data` array:
...
| Element | Description | Required? | Type | Notes |
|----|----|----|----|----|
| clocks | Clock names to register as empty TimeFrames before loading data | No | array of strings | Each name must be populated by a later `data_type: "time"` entry (or equivalent). Load fails if any declared clock is still empty after all entries are processed. |

Data entries may reference a `clock` before that clock is populated. When the clock is already declared in `clocks`, data is registered on that clock immediately; otherwise clock bindings are deferred until after all `data` entries are loaded. The implicit default `time` clock is not validated unless it appears in `clocks`.

### Spike2 (`.smr` / `.smrx`)

See the [Spike2 (CED) acquisition guide](../acquisition_systems/spike2.qmd) for SonPy setup, ADC analog/event/interval loading, and clock assignment examples.

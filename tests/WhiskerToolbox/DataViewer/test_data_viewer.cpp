#include "AnalogTimeSeries/AnalogSeriesHelpers.hpp"
...
#include "LayoutCalculator/LayoutCalculator.hpp"
...
// ============================================================================
// Helper functions for creating CorePlotting parameter structs
// ============================================================================

/**
 * @brief Create AnalogSeriesMatrixParams from display options and plotting manager
 */
CorePlotting::AnalogSeriesMatrixParams makeAnalogParams(
        NewAnalogTimeSeriesDisplayOptions const & options,
        LayoutCalculator const & manager) {

    CorePlotting::AnalogSeriesMatrixParams params;
    params.allocated_y_center = options.layout.allocated_y_center;
    params.allocated_height = options.layout.allocated_height;
    params.intrinsic_scale = options.scaling.intrinsic_scale;
    params.user_scale_factor = options.user_scale_factor;
    params.global_zoom = manager.getGlobalZoom();
    params.user_vertical_offset = options.scaling.user_vertical_offset;
    params.data_mean = options.data_cache.cached_mean;
    params.std_dev = options.data_cache.cached_std_dev;
    params.global_vertical_scale = manager.getGlobalVerticalScale();
    return params;
}

/**
 * @brief Create ViewProjectionParams from plotting manager
 */
CorePlotting::ViewProjectionParams makeViewParams(LayoutCalculator const & manager) {
    CorePlotting::ViewProjectionParams params;
    params.vertical_pan_offset = manager.getPanOffset();
    params.viewport_y_min = -1.0f;// Standard NDC range
    params.viewport_y_max = 1.0f;
    return params;
}

/**
 * @brief Create IntervalSeriesMatrixParams from display options and plotting manager
 */
CorePlotting::IntervalSeriesMatrixParams makeIntervalParams(
        NewDigitalIntervalSeriesDisplayOptions const & options,
        LayoutCalculator const & manager) {

    CorePlotting::IntervalSeriesMatrixParams params;
    params.allocated_y_center = options.layout.allocated_y_center;
    params.allocated_height = options.layout.allocated_height;
    params.margin_factor = options.margin_factor;
    params.global_zoom = manager.getGlobalZoom();
    params.global_vertical_scale = manager.getGlobalVerticalScale();
    params.extend_full_canvas = options.extend_full_canvas;
    return params;
}

/**
 * @brief Create EventSeriesMatrixParams from display options and plotting manager
 */
CorePlotting::EventSeriesMatrixParams makeEventParams(
        NewDigitalEventSeriesDisplayOptions const & options,
        LayoutCalculator const & manager,
        float viewport_y_min,
        float viewport_y_max) {

    CorePlotting::EventSeriesMatrixParams params;
    params.allocated_y_center = options.layout.allocated_y_center;
    params.allocated_height = options.layout.allocated_height;
    params.event_height = options.event_height;
    params.margin_factor = options.margin_factor;
    params.global_vertical_scale = manager.getGlobalVerticalScale();
    params.viewport_y_min = viewport_y_min;
    params.viewport_y_max = viewport_y_max;
    params.plotting_mode = (options.plotting_mode == EventPlottingMode::FullCanvas)
...
TEST_CASE("Integration Test: Mixed Data Types with Coordinate Allocation and Panning", "[integration][mvp][mixed_data]") {
    ...
    SECTION("Multi-series coordinate allocation and panning behavior") {
        ...
        int gaussian_series = 0;
        int uniform_series = 1;
        ...
        // Test coordinate allocation without panning
        {
            ...
            glm::mat4 gaussian_model = CorePlotting::getAnalogModelMatrix(makeAnalogParams(gaussian_options, manager));
            glm::mat4 gaussian_view = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
            glm::mat4 gaussian_projection = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);

            glm::mat4 uniform_model = CorePlotting::getAnalogModelMatrix(makeAnalogParams(uniform_options, manager));
            glm::mat4 uniform_view = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
            glm::mat4 uniform_projection = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);

            glm::mat4 interval_model = CorePlotting::getIntervalModelMatrix(makeIntervalParams(interval_options, manager));
            glm::mat4 interval_view = CorePlotting::getIntervalViewMatrix(makeViewParams(manager));
            glm::mat4 interval_projection = CorePlotting::getIntervalProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);
...

        {
            ...
            glm::mat4 gaussian_model_panned = CorePlotting::getAnalogModelMatrix(makeAnalogParams(gaussian_options, manager));
            glm::mat4 gaussian_view_panned = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
            glm::mat4 gaussian_projection_panned = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);

            glm::mat4 uniform_model_panned = CorePlotting::getAnalogModelMatrix(makeAnalogParams(uniform_options, manager));
            glm::mat4 uniform_view_panned = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
            glm::mat4 uniform_projection_panned = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);

            glm::mat4 interval_model_panned = CorePlotting::getIntervalModelMatrix(makeIntervalParams(interval_options, manager));
            glm::mat4 interval_view_panned = CorePlotting::getIntervalViewMatrix(makeViewParams(manager));
            glm::mat4 interval_projection_panned = CorePlotting::getIntervalProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);
        ...
        {
            glm::mat4 gaussian_model_neg = CorePlotting::getAnalogModelMatrix(makeAnalogParams(gaussian_options, manager));
            glm::mat4 gaussian_view_neg = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
            glm::mat4 gaussian_projection_neg = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);

            glm::mat4 interval_model_neg = CorePlotting::getIntervalModelMatrix(makeIntervalParams(interval_options, manager));
            glm::mat4 interval_view_neg = CorePlotting::getIntervalViewMatrix(makeViewParams(manager));
            glm::mat4 interval_projection_neg = CorePlotting::getIntervalProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f);
...
TEST_CASE("Integration Test: Mixed Analog and Digital Event Series", "[integration][mvp][mixed_analog_events]") {
    ...
    SECTION("Global stacked allocation: 2 analog + 2 digital events = 4 series each getting 1/4 canvas") {
        ...
        glm::mat4 analog1_model = CorePlotting::getAnalogModelMatrix(makeAnalogParams(analog1_options, manager));
        glm::mat4 analog2_model = CorePlotting::getAnalogModelMatrix(makeAnalogParams(analog2_options, manager));
        glm::mat4 event1_model = CorePlotting::getEventModelMatrix(makeEventParams(event1_options, manager, -1.0f, 1.0f));
        glm::mat4 event2_model = CorePlotting::getEventModelMatrix(makeEventParams(event2_options, manager, -1.0f, 1.0f));
        ...
        glm::vec2 analog1_mean_point = applyMVPTransformationIntegration(5000, analog1_options.data_cache.cached_mean, analog1_model,
                                                                         CorePlotting::getAnalogViewMatrix(makeViewParams(manager)),
                                                                         CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(1), TimeFrameIndex(10000), -1.0f, 1.0f));
        ...
        glm::mat4 analog1_view_panned = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        glm::mat4 event1_view_panned = CorePlotting::getEventViewMatrix(makeEventParams(event1_options, manager, -1.0f, 1.0f), makeViewParams(manager));
        glm::mat4 event2_view_panned = CorePlotting::getEventViewMatrix(makeEventParams(event2_options, manager, -1.0f, 1.0f), makeViewParams(manager));
...
TEST_CASE("X-Axis Extreme Range Scrolling Test", "[integration][xaxis][extreme_range]") {
    SECTION("Full range to normal window transition with data validation") {
        ...
        glm::mat4 model_full = CorePlotting::getAnalogModelMatrix(makeAnalogParams(display_options, manager));
        glm::mat4 view_full = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        glm::mat4 projection_full = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(data_start), TimeFrameIndex(data_end), -400.0f, 400.0f);
        ...
        glm::mat4 model_normal = CorePlotting::getAnalogModelMatrix(makeAnalogParams(display_options, manager));
        glm::mat4 view_normal = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        glm::mat4 projection_normal = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(normal_start), TimeFrameIndex(normal_end), -400.0f, 400.0f);
        ...
        glm::mat4 projection_tiny = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(tiny_start), TimeFrameIndex(tiny_end), -400.0f, 400.0f);
        ...
        glm::mat4 projection_recovery = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(data_start), TimeFrameIndex(data_end), -400.0f, 400.0f);
    ...
    SECTION("Short video extreme scrolling simulation (704 frames)") {
        ...
        glm::mat4 projection_full = CorePlotting::getAnalogProjectionMatrix(
                TimeFrameIndex(x_axis.getStart()), TimeFrameIndex(x_axis.getEnd()), -400.0f, 400.0f);
        ...
        glm::mat4 projection_tiny = CorePlotting::getAnalogProjectionMatrix(
                TimeFrameIndex(x_axis.getStart()), TimeFrameIndex(x_axis.getEnd()), -400.0f, 400.0f);
        ...
        glm::mat4 projection_2samples = CorePlotting::getAnalogProjectionMatrix(
                TimeFrameIndex(x_axis.getStart()), TimeFrameIndex(x_axis.getEnd()), -400.0f, 400.0f);
        ...
        for (size_t i = 0; i < test_ranges.size(); ++i) {
            ...
            glm::mat4 projection = CorePlotting::getAnalogProjectionMatrix(
                    TimeFrameIndex(x_axis.getStart()), TimeFrameIndex(x_axis.getEnd()), -400.0f, 400.0f);
    ...
    SECTION("XAxis integration with LayoutCalculator") {
        // Test the integration between XAxis class and LayoutCalculator
        ...
        // Create layout calculator and set it to match XAxis
        LayoutCalculator manager;

        // Test synchronized range changes
        INFO("Testing synchronized XAxis and LayoutCalculator range changes");
    ...
    SECTION("MVP Matrix corruption with extreme ranges - testing for NaN/Infinity") {
        ...
        auto model_huge = CorePlotting::getAnalogModelMatrix(makeAnalogParams(display_options, manager));
        auto view_huge = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        auto projection_huge = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(huge_start), TimeFrameIndex(huge_end), -1000.0f, 1000.0f);
        ...
        auto model_tiny = CorePlotting::getAnalogModelMatrix(makeAnalogParams(display_options, manager));
        auto view_tiny = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        auto projection_tiny = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(tiny_start), TimeFrameIndex(tiny_end), -1000.0f, 1000.0f);
        ...
        auto model_inv = CorePlotting::getAnalogModelMatrix(makeAnalogParams(display_options, manager));
        auto view_inv = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        auto projection_inv = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(inv_start), TimeFrameIndex(inv_end), -1000.0f, 1000.0f);
        ...
        auto model_zero_std = CorePlotting::getAnalogModelMatrix(makeAnalogParams(constant_options, manager));
        auto view_zero_std = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        auto projection_zero_std = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(0), TimeFrameIndex(999), -1000.0f, 1000.0f);
        ...
        auto model_recovery = CorePlotting::getAnalogModelMatrix(makeAnalogParams(display_options, manager));
        auto view_recovery = CorePlotting::getAnalogViewMatrix(makeViewParams(manager));
        auto projection_recovery = CorePlotting::getAnalogProjectionMatrix(TimeFrameIndex(100), TimeFrameIndex(200), -1000.0f, 1000.0f);

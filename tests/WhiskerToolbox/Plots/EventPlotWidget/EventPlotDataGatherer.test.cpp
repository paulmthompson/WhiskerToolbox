...
std::shared_ptr<DataManager> createTestDataManager() {
    ...
    dm->setTime(time_key, createTimeFrame(0, 600));
...
TEST_CASE("EventPlotDataGatherer - realistic raster plot scenario", "[EventPlotDataGatherer]") {
    ...
    SECTION("simulates neural spike raster with multiple trials") {
        ...
        auto spikes = createEventSeries({// Trial 1 region [0, 1000): response around t=300-400
                                         310, 320, 350, 380, 390,
        ...
        auto trials = createIntervalSeries({{0, 1000},
                                            {1000, 2000},
                                            {2000, 3000}});
    ...
    SECTION("sparse spike data across many trials") {
        ...
        auto trials = createIntervalSeries({{0, 200}, {200, 400}, {400, 600}, {600, 800}, {800, 1000}});

/**
 * @file HeatmapPropertiesWidget.test.cpp
 * @brief Unit tests for HeatmapPropertiesWidget
 * 
 * Tests the properties widget for HeatmapWidget, including:
...
#include "Plots/HeatmapWidget/UI/HeatmapPropertiesWidget.hpp"
...
TEST_CASE("HeatmapPropertiesWidget combo box population", "[HeatmapPropertiesWidget]")
{
...
TEST_CASE("HeatmapPropertiesWidget observer callback", "[HeatmapPropertiesWidget]")
{
    // Create QApplication if it doesn't exist (required for Qt widgets)
    int argc = 0;
    QApplication app(argc, nullptr);
    ...
    SECTION("combo box refreshes when interval series is added") {
        auto data_manager = std::make_shared<DataManager>();
        auto state = std::make_shared<HeatmapState>();
        ...
        interval_series->setTimeFrame(time_frame);
        data_manager->setData<DigitalIntervalSeries>("new_intervals", interval_series, TimeKey("time"));
        ...
        // Combo box should now be populated
        REQUIRE(combo->isEnabled());
        REQUIRE(combo->count() >= 2);  // At least 1 interval plus "(None)"
        bool found = false;
        for (int i = 0; i < combo->count(); ++i) {
            if (combo->itemText(i).toStdString() == "new_intervals") {
                found = true;
                break;
            }
        }
...
TEST_CASE("HeatmapPropertiesWidget cleanup", "[HeatmapPropertiesWidget]")
{
    // Create QApplication if it doesn't exist (required for Qt widgets)
    int argc = 0;
    QApplication app(argc, nullptr);

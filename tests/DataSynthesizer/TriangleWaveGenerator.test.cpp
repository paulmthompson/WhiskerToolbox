...
TEST_CASE("TriangleWave produces correct output size", "[TriangleWave]") {
    auto ts = runTriangleWave(R"({"num_samples": 150, "amplitude": 1.0, "frequency": 0.01})");
    REQUIRE(ts->getNumSamples() == 150);
}

TEST_CASE("TriangleWave value at t=0 with default phase is ~0", "[TriangleWave]") {
    auto ts = runTriangleWave(R"({"num_samples": 100, "amplitude": 1.0, "frequency": 0.01})");
...
TEST_CASE("TriangleWave values are bounded within [-amplitude, amplitude]", "[TriangleWave]") {
    ...
    auto ts = runTriangleWave(R"({"num_samples": 1000, "amplitude": 2.5, "frequency": 0.01})");
    auto values = ts->getAnalogTimeSeries();
    for (auto v: values) {
...
TEST_CASE("TriangleWave one complete cycle reaches +amplitude and -amplitude", "[TriangleWave]") {
    ...
    // 1000 samples, frequency = 0.001 → one complete cycle
    auto ts = runTriangleWave(R"({"num_samples": 1000, "amplitude": 3.0, "frequency": 0.001})");
    auto values = ts->getAnalogTimeSeries();
...
TEST_CASE("TriangleWave dc_offset shifts all values", "[TriangleWave]") {
    float const dc = -2.0f;
    auto ts_no_dc = runTriangleWave(R"({"num_samples": 100, "amplitude": 1.0, "frequency": 0.01})");
    auto ts_with_dc = runTriangleWave(
            R"({"num_samples": 100, "amplitude": 1.0, "frequency": 0.01, "dc_offset": -2.0})");
...
TEST_CASE("TriangleWave is linear between zero-crossings", "[TriangleWave]") {
    // 1000 samples, frequency = 0.001 → one complete cycle over 1000 samples
    // Peak at t=250, zero-crossing down at ~t=500, trough at t=750
    auto ts = runTriangleWave(R"({"num_samples": 1000, "amplitude": 1.0, "frequency": 0.001})");
    auto values = ts->getAnalogTimeSeries();
...
TEST_CASE("TriangleWave rejects num_samples <= 0", "[TriangleWave]") {
    auto result = GeneratorRegistry::instance().generate(
            "TriangleWave", R"({"num_samples": 0, "amplitude": 1.0, "frequency": 0.01})");
    REQUIRE_FALSE(result.has_value());
}

TEST_CASE("TriangleWave rejects frequency <= 0", "[TriangleWave]") {
    auto result = GeneratorRegistry::instance().generate(
            "TriangleWave", R"({"num_samples": 100, "amplitude": 1.0, "frequency": -1.0})");
    REQUIRE_FALSE(result.has_value());
}

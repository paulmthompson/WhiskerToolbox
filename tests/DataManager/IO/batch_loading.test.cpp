...
TEST_CASE_METHOD(BatchLoadingTestFixture,
    ...
    SECTION("supportsBatchLoading returns true for dlc_csv format with Points") {
        REQUIRE(loader.supportsBatchLoading("dlc_csv", DM_DataType::Points));
    }
    ...
    SECTION("supportsBatchLoading returns true for csv format with Points too") {
        ...
        REQUIRE(loader.supportsBatchLoading("csv", DM_DataType::Points));
    }
...
TEST_CASE_METHOD(BatchLoadingTestFixture,
    ...
    SECTION("supportsBatchLoading returns true for csv DigitalEvent") {
        REQUIRE(loader.supportsBatchLoading("csv", DM_DataType::DigitalEvent));
    }
...
TEST_CASE_METHOD(BatchLoadingTestFixture,
    ...
    SECTION("supportsBatchLoading returns true for binary Analog") {
        REQUIRE(loader.supportsBatchLoading("binary", DM_DataType::Analog));
    }
...
TEST_CASE_METHOD(BatchLoadingTestFixture,
    ...
    SECTION("Registry reports batch loading support correctly") {
        ...
        REQUIRE(registry.isBatchLoadingSupported("dlc_csv", DM_DataType::Points));

        // Binary analog should support batch loading
        REQUIRE(registry.isBatchLoadingSupported("binary", DM_DataType::Analog));

        // CSV digital event should support batch loading
        REQUIRE(registry.isBatchLoadingSupported("csv", DM_DataType::DigitalEvent));

        // Regular CSV line should NOT support batch loading
        REQUIRE_FALSE(registry.isBatchLoadingSupported("csv", DM_DataType::Line));
    }
    ...
    SECTION("tryLoadBatch works through registry for DLC files") {
        ...
        BatchLoadResult result = registry.tryLoadBatch("dlc_csv", DM_DataType::Points,

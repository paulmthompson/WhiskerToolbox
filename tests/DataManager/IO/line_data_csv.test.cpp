...
protected:
    void createTestLineData() {
        ...
        std::vector<Point2D<float>> const line1_points = {
                {10.0f, 20.0f},
                {30.0f, 40.0f},
                {50.0f, 60.0f}};
        Line2D const line1(line1_points);

        // Line 2 at time frame 0: L-shaped line
        std::vector<Point2D<float>> const line2_points = {
                {100.0f, 100.0f},
                {150.0f, 100.0f},
                {150.0f, 150.0f}};
        Line2D const line2(line2_points);

        // Line 3 at time frame 1: Zigzag line
        std::vector<Point2D<float>> const line3_points = {
                {200.0f, 200.0f},
                {250.0f, 250.0f},
                {300.0f, 200.0f},
                {350.0f, 250.0f}};
        Line2D const line3(line3_points);
        ...
        return R"([
{
    ...
    void verifyLineDataEquality(LineData const & loaded_data) const {
        ...
        for (size_t i = 0; i < original_times_vec.size(); ++i) {
            ...
            TimeFrameIndex const time = original_times_vec[i];
            auto const & original_lines = original_line_data->getAtTime(time);
            auto const & loaded_lines = loaded_data.getAtTime(time);
            ...
            for (size_t line_idx = 0; line_idx < original_lines.size(); ++line_idx) {
                Line2D const & original_line = original_lines[line_idx];
                Line2D const & loaded_line = loaded_lines[line_idx];
                ...
                for (size_t point_idx = 0; point_idx < original_line.size(); ++point_idx) {
                    ...
                    Point2D<float> const loaded_point = loaded_line[point_idx];
...
TEST_CASE_METHOD(LineDataCSVTestFixture, "DM - IO - LineData - CSV save and load through direct functions", "[LineData][CSV][IO]") {
    ...
    SECTION("Save LineData to CSV format") {
        bool const save_success = saveCSVLineData();
...
TEST_CASE_METHOD(LineDataCSVTestFixture, "DM - IO - LineData - CSV load through DataManager JSON config", "[LineData][CSV][IO][DataManager]") {
    ...
    SECTION("Load CSV LineData through DataManager") {
        ...
        std::string const json_config = createJSONConfig();
        std::filesystem::path const json_filepath = test_dir / "config.json";
    ...
    SECTION("DataManager handles missing CSV file gracefully") {
        ...
        std::filesystem::path const fake_filepath = test_dir / "nonexistent.csv";
        std::string const json_config = R"([
{
    "data_type": "line",
    ...
    "format": "csv"
        ...
        std::filesystem::path const json_filepath = test_dir / "config_missing.json";

 ...
 *
 * Binary I/O round-trip preserves stored TimeFrameIndex (sample indices).
 */
...
void checkStoredInterval(
    ...
    REQUIRE(stored.start == TimeFrameIndex(start));
    REQUIRE(stored.end == TimeFrameIndex(end));
}
}// anonymous namespace
...
TEST_CASE("DigitalInterval Binary Integration - Single Channel Rising Edge",
    ...
    SECTION("Simple TTL pulses") {
        ...
        json config = json::array({{{"data_type", "digital_interval"},
                                    {"name", "simple_ttl"},
...
TEST_CASE("DigitalInterval Binary Integration - Multiple Channels",
    ...
    SECTION("Channel 0 and Channel 5 in same file") {
        ...
        // Load channel 0
        json config_ch0 = json::array({{{"data_type", "digital_interval"},
                                        {"name", "ch0_intervals"},
                                        {"filepath", binary_path.string()},
                                        {"format", "uint16"},
                                        {"channel", 0},
        ...
        json config_ch5 = json::array({{{"data_type", "digital_interval"},
                                        {"name", "ch5_intervals"},
                                        {"filepath", binary_path.string()},
                                        {"format", "uint16"},
                                        {"channel", 5},
...
TEST_CASE("DigitalInterval Binary Integration - Falling Edge Detection",
    ...
    SECTION("Falling edge creates inverted intervals") {
        ...
        checkStoredInterval(*loaded, 0, 20, 50);
        checkStoredInterval(*loaded, 1, 60, 100);
    }
}
...
TEST_CASE("DigitalInterval Binary Integration - Edge Cases",
    ...
    SECTION("Adjacent pulses") {
        ...
        checkStoredInterval(*loaded, 0, 5, 45);
    }

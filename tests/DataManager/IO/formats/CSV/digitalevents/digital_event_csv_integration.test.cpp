...
TEST_CASE("DigitalEvent CSV Integration - Scaling Options",
    ...
    SECTION("Scale multiply") {
        ...
        for (std::size_t i = 0; i < original->size(); ++i) {
            int64_t const expected = original->getStoredEvent(i).getValue() * 2;
            REQUIRE(loaded->getStoredEvent(i).getValue() == expected);
        }
    }
    ...
    SECTION("Scale divide") {
        ...
        for (std::size_t i = 0; i < original->size(); ++i) {
            int64_t const expected = original->getStoredEvent(i).getValue() / 10;
            REQUIRE(loaded->getStoredEvent(i).getValue() == expected);
        }
    }
    ...
    SECTION("Scale sub-1.0 float timestamps to sample indices") {
        ...
        REQUIRE(loaded->getStoredEvent(0).getValue() == 447);

        // 0.01670 * 30000 = 501.0 → 501
        REQUIRE(loaded->getStoredEvent(1).getValue() == 501);

        // 0.10957 * 30000 = 3287.1 → 3287
        REQUIRE(loaded->getStoredEvent(2).getValue() == 3287);

        // 0.37670 * 30000 = 11301.0 → 11301
        REQUIRE(loaded->getStoredEvent(3).getValue() == 11301);
    }
}

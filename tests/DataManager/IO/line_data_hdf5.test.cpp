...
TEST_CASE_METHOD(LineDataHDF5TestFixture, "DM - IO - LineData - HDF5 loader registration and JSON config parsing", "[LineData][HDF5][IO][DataManager]") {
    ...
    SECTION("Test HDF5 loader registration and JSON config handling") {
        ...
        REQUIRE(registry.isFormatSupported("hdf5", DM_DataType::Line));
        ...
        REQUIRE_FALSE(registry.isFormatSupported("hdf5", DM_DataType::Line));
...
TEST_CASE_METHOD(LineDataHDF5TestFixture, "DM - IO - LineData - HDF5 loader registration and format support", "[LineData][HDF5][IO]") {
    ...
    SECTION("Verify HDF5 loader is registered and supports line format") {
        ...
        auto & registry = LoaderRegistry::getInstance();
        bool hdf5_supported = registry.isFormatSupported("hdf5", DM_DataType::Line);
...
// This should be true if HDF5 is enabled at compile time
    ...
    SECTION("Check HDF5 in supported formats list") {
        ...
        auto supported_formats = registry.getSupportedFormats(DM_DataType::Line);

#ifdef ENABLE_HDF5
        // HDF5 should be in the list of supported formats
        bool found_hdf5 = std::find(supported_formats.begin(), supported_formats.end(), "hdf5") != supported_formats.end();
        REQUIRE(found_hdf5);
#endif

        // CSV should always be supported (internal plugin)
        bool found_csv = std::find(supported_formats.begin(), supported_formats.end(), "csv") != supported_formats.end();
        REQUIRE(found_csv);
    }
}

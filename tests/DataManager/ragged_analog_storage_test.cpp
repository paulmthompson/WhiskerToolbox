...
#include "AnalogTimeSeries/storage/RaggedAnalogStorage.hpp"
...
TEST_CASE("OwningRaggedAnalogStorage out-of-order append preserves getTimeRange",
          "[RaggedAnalogStorage][out_of_order][regression]") {
    OwningRaggedAnalogStorage storage;

    SECTION("Minimal repro: future frame, intermediate frame, future frame again") {
        storage.append(TimeFrameIndex{50}, 50.0f);
        storage.append(TimeFrameIndex{10}, 10.0f);
        storage.append(TimeFrameIndex{50}, 50.1f);
        ...
        auto span = storage.getValuesAtTime(TimeFrameIndex{50});
        REQUIRE(span.size() == 2);
        CHECK(span[0] == 50.0f);
        CHECK(span[1] == 50.1f);
        ...
        CHECK(storage.getTime(mid_start) == TimeFrameIndex{10});
    }

    SECTION("NeuroSAM workflow: label frame 50 then batch-write frames 1-100") {
        storage.append(TimeFrameIndex{50}, 0.0f);

        for (int frame = 1; frame <= 100; ++frame) {
            storage.append(TimeFrameIndex{frame}, static_cast<float>(frame));
        }
        ...
        REQUIRE(mid_end - mid_start == 1);
        CHECK(storage.getTime(mid_start) == TimeFrameIndex{25});
    }
}
...
TEST_CASE("LazyRaggedAnalogStorage basic operations", "[RaggedAnalogStorage][lazy]") {
    ...
    auto transform_view = source_data | std::views::transform([](auto const & pair) {
...
TEST_CASE("RaggedAnalogTimeSeries lazy storage", "[RaggedAnalogTimeSeries][lazy]") {
    ...
    SECTION("createFromView creates lazy storage") {
        ...
        auto scaled_vec = elements_vec | std::views::transform([](auto pair) {
...
TEST_CASE("RaggedAnalogTimeSeries range constructor", "[RaggedAnalogTimeSeries]") {
    ...
    SECTION("Construct from transformed view") {
        ...
        auto transformed = indices | std::views::transform([](int i) {

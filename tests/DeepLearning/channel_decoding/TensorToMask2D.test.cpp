...
TEST_CASE("TensorToMask2D - basic thresholding", "[channel_decoding][TensorToMask2D]") {
    ...
    auto const mask = decoder.decode(tensor, ctx, params);
...
TEST_CASE("TensorToMask2D - batch index", "[channel_decoding][TensorToMask2D]") {
    ...
    ctx.batch_index = 0;
    auto const mask0 = decoder.decode(tensor, ctx, params);
    REQUIRE(mask0.size() == 1);
    CHECK(mask0[0].x == 3);
    CHECK(mask0[0].y == 2);

    ctx.batch_index = 1;
    auto const mask1 = decoder.decode(tensor, ctx, params);

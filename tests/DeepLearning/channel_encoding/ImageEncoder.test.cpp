...
TEST_CASE("ImageEncoder - grayscale uint8 basic encoding", "[channel_encoding][ImageEncoder]") {
    ...
    dl::ImageEncoderParams params;
    params.normalize = true;

    encoder.encode(image, src_size, 1, tensor, ctx, params);
...
TEST_CASE("ImageEncoder - RGB uint8 encoding", "[channel_encoding][ImageEncoder]") {
    ...
    encoder.encode(image, src_size, 3, tensor, ctx, params);
...
TEST_CASE("ImageEncoder - batch index writing", "[channel_encoding][ImageEncoder]") {
    ...
    dl::EncoderContext ctx;
    ctx.target_channel = 0;
    ctx.batch_index = 1;// write to second batch element
    ctx.height = 2;
    ctx.width = 2;
...
TEST_CASE("ImageEncoder - invalid channel count throws", "[channel_encoding][ImageEncoder]") {
    ...
    CHECK_THROWS_AS(encoder.encode(image, {4, 4}, 2, tensor, ctx, params), std::invalid_argument);
}
...
TEST_CASE("ImageEncoder - size mismatch throws", "[channel_encoding][ImageEncoder]") {
    ...
    CHECK_THROWS_AS(encoder.encode(image, {2, 2}, 1, tensor, ctx, params), std::invalid_argument);
}

 ...
 */
SeriesLayout createLayout(float y_center, float height, std::string const & id = "test", int index = 0) {
...
TEST_CASE("MappedElement - Basic operations", "[Mappers][MappedElement]") {
    ...
    SECTION("Position conversion") {
        MappedElement const elem{3.0f, 4.0f, EntityId{1}};
...
TEST_CASE("MappedRectElement - Basic operations", "[Mappers][MappedRectElement]") {
    SECTION("Value construction") {
        MappedRectElement const rect{10.0f, 20.0f, 100.0f, 50.0f, EntityId{99}};
    ...
    SECTION("Bounds conversion") {
        MappedRectElement const rect{5.0f, 10.0f, 20.0f, 30.0f, EntityId{1}};
    ...
    SECTION("Center calculation") {
        MappedRectElement const rect{0.0f, 0.0f, 100.0f, 50.0f, EntityId{1}};
...
TEST_CASE("MappedVertex - Basic operations", "[Mappers][MappedVertex]") {
    SECTION("Value construction") {
        MappedVertex const v{1.0f, 2.0f};
        REQUIRE(v.x == 1.0f);
        REQUIRE(v.y == 2.0f);
    }

    SECTION("Position conversion") {
        MappedVertex const v{5.0f, 10.0f};
        auto pos = v.position();
        REQUIRE(pos.x == 5.0f);
        REQUIRE(pos.y == 10.0f);
    }
...
TEST_CASE("RasterMapper::mapEventsRelative", "[Mappers][RasterMapper]") {
    ...
    DigitalEventSeries events({TimeFrameIndex{4},
                               TimeFrameIndex{5},
                               TimeFrameIndex{6},
                               TimeFrameIndex{7}});
    events.setTimeFrame(tf);
    ...
    ClockTicks const ref_abs_time = tf->getTimeAtIndex(TimeFrameIndex{5});// Reference at time 50

    SECTION("Computes relative time") {
        auto mapped = RasterMapper::mapEventsRelative(events, layout, *tf, ref_abs_time.getValue());
...
TEST_CASE("RasterMapper::mapEventsInWindow", "[Mappers][RasterMapper]") {
    ...
    ClockTicks const ref_abs_time = tf->getTimeAtIndex(TimeFrameIndex{5});// time 50

    SECTION("Filters events outside window") {
        // Window: [50-30, 50+20] = [20, 70]
        auto mapped = RasterMapper::mapEventsInWindow(
                events, layout, *tf, ref_abs_time.getValue(), 30, 20);
...
TEST_CASE("RasterMapper::computeRowYCenter", "[Mappers][RasterMapper]") {
    SECTION("Single row") {
        float const y = RasterMapper::computeRowYCenter(0, 1, -1.0f, 1.0f);
        REQUIRE(y == 0.0f);// Center of [-1, 1]
    }

    SECTION("Two rows") {
        float const y0 = RasterMapper::computeRowYCenter(0, 2, -1.0f, 1.0f);
        float const y1 = RasterMapper::computeRowYCenter(1, 2, -1.0f, 1.0f);
    ...
    SECTION("Four rows") {
        float const y0 = RasterMapper::computeRowYCenter(0, 4, 0.0f, 1.0f);
        float const y1 = RasterMapper::computeRowYCenter(1, 4, 0.0f, 1.0f);
        float const y2 = RasterMapper::computeRowYCenter(2, 4, 0.0f, 1.0f);
        float const y3 = RasterMapper::computeRowYCenter(3, 4, 0.0f, 1.0f);
...
TEST_CASE("RasterMapper::makeRowLayout", "[Mappers][RasterMapper]") {
    ...
    float const expected_y = RasterMapper::computeRowYCenter(1, 4, -1.0f, 1.0f);
...
TEST_CASE("SpatialMapper::mapPoint", "[Mappers][SpatialMapper]") {
    Point2D<float> const pt{10.0f, 20.0f};
...
TEST_CASE("RasterMapper - negative x for pre-alignment events",
    ...
    SECTION("single event before reference produces negative x") {
        ...
        TimeFrameIndex const reference_idx{11};
        ClockTicks const ref_abs_time = tf->getTimeAtIndex(reference_idx);
    ...
    SECTION("mix of pre- and post-alignment events") {
        ...
        DigitalEventSeries events({TimeFrameIndex{8},
                                   TimeFrameIndex{10},
                                   TimeFrameIndex{12}});
        events.setTimeFrame(tf);

        TimeFrameIndex const reference_idx{10};
        ClockTicks const ref_abs_time = tf->getTimeAtIndex(reference_idx);
    ...
    SECTION("all events before reference produce all-negative x") {
        ...
        DigitalEventSeries events({TimeFrameIndex{95},
                                   TimeFrameIndex{96},
                                   TimeFrameIndex{97},
                                   TimeFrameIndex{98},
                                   TimeFrameIndex{99}});
        ...
        TimeFrameIndex const reference_idx{100};
        ClockTicks const ref_abs_time = tf->getTimeAtIndex(reference_idx);
...
TEST_CASE("RasterMapper::mapEventsInWindow - includes pre-alignment events",
    ...
    ClockTicks const ref_abs_time = tf->getTimeAtIndex(TimeFrameIndex{11});// alignment event

    SECTION("event at 10 maps to x=-1 within window") {
        auto mapped = RasterMapper::mapEventsInWindow(
                events, layout, *tf, ref_abs_time.getValue(), 5, 5);
    ...
    SECTION("wider window captures all events with correct signs") {
        ...
        auto mapped = RasterMapper::mapEventsInWindow(
                events, layout, *tf, ref_abs_time.getValue(), 25, 25);
 ...
 *
 *   1. Create a GatherResult from prepared windows and companion alignment events
...
TEST_CASE("GatherResult → RasterMapper produces negative x for pre-alignment events",
    ...
    SECTION("user scenario: events at [10,20,30], alignment at [11,21,31]") {
        ...
        auto windows = createWindowsAroundEvents(alignment, 5, 5);
        auto gathered = gather(source, windows, alignment);
        ...
        for (size_t trial = 0; trial < gathered.size(); ++trial) {
            ...
            auto ref_abs_time = static_cast<int>(gathered.alignmentTimeAt(trial).getValue());
    ...
    SECTION("mixed pre/post events through full pipeline") {
        ...
        auto windows = createWindowsAroundEvents(alignment, 50, 50);
        auto gathered = gather(source, windows, alignment);

        REQUIRE(gathered.size() == 1);
        REQUIRE(gathered[0]->size() == 3);

        auto ref_abs_time = static_cast<int>(gathered.alignmentTimeAt(0).getValue());
        auto mapped = RasterMapper::mapEventsInWindow(
    ...
    SECTION("cross-TimeFrame: events and alignment on different TimeFrames") {
        ...
        CHECK(gathered.alignmentTimeAt(0) == ClockTicks(11));
        CHECK(gathered.alignmentTimeAt(1) == ClockTicks(21));
        CHECK(gathered.alignmentTimeAt(2) == ClockTicks(31));
...
TEST_CASE("Projection consistency: canvas vs axis widget coordinate systems",
    ...
    auto const x_range = static_cast<float>(vsd.x_max - vsd.x_min);
    float const x_center = static_cast<float>(vsd.x_min + vsd.x_max) / 2.0f;
    auto const y_range = static_cast<float>(vsd.y_max - vsd.y_min);
    ...
    float const canvas_screen_x = (origin_ndc.x + 1.0f) * 0.5f * static_cast<float>(vp_width);
    ...
    glm::vec2 const axis_screen = CorePlotting::worldToScreen(runtime_vs, 0.0f, 0.0f);
    ...
    float const canvas_screen_100 = (pos100_ndc.x + 1.0f) * 0.5f * static_cast<float>(vp_width);
